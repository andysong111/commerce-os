...
test("ZIP generates v0.2.3 worker and checks all declared files", async () => {
  const r = await readFile("src/app/api/shopling-stock-state-sync/download/route.ts", "utf8");
  assert.match(r, /const VERSION = "0\.2\.3"/); assert.match(r, /buildStockWorker/);
  assert.match(r, /content-shopling-v023\.js/); assert.match(r, /missing_packaged_file/);
