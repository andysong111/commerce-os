...
import {
  ...
  listWorkflowRuns,
} from "../src/lib/githubActionsRuns.ts";
  ...
  const response = await postDispatch(
    new Request("http://localhost/api/engine-runners/dispatch", {
      ...
      body: JSON.stringify({
        ...
        inputs: { product_code: "BATH001", source_link: "https://example.com" },
      }),
    }),
  );
  const body = await response.json();
...
test("Keyword Engine Runner page renders safety banner and expected artifacts", async () => {
  const page = await readFile(
    new URL("../src/app/keyword-engine-runner/page.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.ok(
    engineRunnerConfigs
      .find((config) => config.kind === "keyword_engine")
      ?.expectedArtifacts.includes("keyword_mvp_approval_sheet.csv"),
  );
  assert.equal(
    engineRunnerConfigs.find((config) => config.kind === "keyword_engine")
...
test("Detail Page Engine Runner page renders safety banner and expected artifacts", async () => {
  const page = await readFile(
    new URL("../src/app/detail-page-engine-runner/page.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.ok(
    engineRunnerConfigs
      .find((config) => config.kind === "detail_page_engine")
      ?.expectedArtifacts.includes("detailpage_final.html"),
  );
  assert.equal(
    engineRunnerConfigs.find((config) => config.kind === "detail_page_engine")
      ?.expectedArtifactName,
    "detail-page-engine-output",
  );
});
...
test("run listing API rejects invalid runner kind", async () => {
  const response = await getRunnerRuns(
    new Request("http://localhost/api/engine-runners/runs?kind=unknown"),
  );
  assert.equal(response.status, 400);
});
...
test("run listing API returns not_configured when token is missing", async () => {
  delete process.env.GITHUB_ENGINE_DISPATCH_TOKEN;
  const response = await getRunnerRuns(
    new Request("http://localhost/api/engine-runners/runs?kind=keyword_engine"),
  );
  const body = await response.json();
...
test("run listing client calls the correct workflow runs URL and does not expose token", async () => {
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      "https://api.github.com/repos/andysong111/andysong111-keyword-engine-soon/actions/workflows/keyword-engine-runner.yml/runs?per_page=10",
    );
...
test("artifact listing client calls the correct artifacts URL and detects expected names", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    return Response.json({
      artifacts: [
        {
          id: 456,
          name: "detail-page-engine-output",
          size_in_bytes: 100,
          expired: false,
          created_at: "2026-06-17T00:00:00Z",
          updated_at: "2026-06-17T00:01:00Z",
  ...
  try {
    ...
    assert.equal(
      calledUrl,
      "https://api.github.com/repos/andysong111/product-detail-page-auto/actions/runs/123/artifacts",
    );
...
test("No local shell execution utility is introduced", async () => {
  ...
  for (const file of files) {
    ...
    assert.doesNotMatch(
      source,
      /child_process|powershell|pwsh|exec\(|spawn\(|Shopling.*apply|publish.*Shopling|1688.*fetch|OpenAI|image generation/i,
    );
  }
});
...
test("client form helper preserves generated detail page product_code for following dispatch", async () => {
  const { persistGeneratedDetailPageProductCode } =
    await import("../src/lib/engineRunnerFormState.ts");
  ...
  persistGeneratedDetailPageProductCode("detail_page_engine", form, {
    inputs: { product_code: "DP-20260618T010203Z-ABC123" },
  });
...
test("source_link-only detail page dispatch works when preview is skipped", async () => {
  ...
  try {
    ...
    assert.equal(response.status, 200);
    assert.match(
      requestBody.inputs.product_code,
      /^DP-\d{8}T\d{6}Z-[A-Z0-9]{6}$/,
    );
