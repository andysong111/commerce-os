...
test("Shopling v0.5.6+ downloadable package requires goods key plus self-code exact identity", async () => {
  ...
  assert.match(source, /manifest\.version = "(?:0\.5\.[6789]|0\.6\.0)"/);
  ...
  assert.match(source, /shopling_v05[6789]_identity_rewrite_failed/);
});
...
test("Shopling v0.5.6+ uses function replacers so generated $& source cannot corrupt the ZIP", async () => {
  ...
  assert.match(source, /shopling_v05[6789]_generated_pipeline_syntax_invalid/);
});
...
test("Shopling v0.5.6+ package carries the field-verified category mapping controls", async () => {
  ...
  assert.match(source, /commerce-os-shopling-account-title-bridge-v(?:0\.5\.[6789]|0\.6\.0)\.zip/);
  assert.match(source, /Commerce OS Shopling Account Title Bridge v(?:0\.5\.[6789]|0\.6\.0)/);
});