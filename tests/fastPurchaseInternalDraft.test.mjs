...
test("only shortage or stockout manual rows with positive quantities can enter a draft", () => {
  assert.match(draft, /current\.action !== "MANUAL_REVIEW" && current\.action !== "DEMAND_ONLY_REVIEW"/);
...
test("browser action requires explicit confirmation before the internal write", () => {
  assert.match(actions, /window\.confirm/);
  assert.match(actions, /내부 발주 Draft/);
