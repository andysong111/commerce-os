...
function buildReservedPlan(
  ...
  for (const market of PRODUCT_GROUP_MARKET_REGISTRY) {
    ...
    plan.push({
      ...
      source_materials: Array.isArray(titleRow.source_materials) ? titleRow.source_materials : [],
    });
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const plan = buildReservedPlan(dispatchId, detail.ledger.common_search_line, reserved, initialGoodsKeys);
    ...
    externalWriteStarted = true;
    externalRequestId = upload.requestId;
    await patchSeoTitleDispatch(context, dispatchId, {
      external_request_id: upload.requestId,
      result_payload: {
        pipelineVersion: SEO_SHOPLING_LIVE_PIPELINE_VERSION,
    ...
    if (reservationCreated && !externalWriteStarted) {
    ...
    if (dispatchCreated) {
      await patchSeoTitleDispatch(context, dispatchId, {
        status: externalWriteStarted ? "submitted" : "cancelled",
        completed_at: externalWriteStarted ? null : new Date().toISOString(),
        ...(externalRequestId ? { external_request_id: externalRequestId } : {}),
        result_payload: {
          pipelineVersion: SEO_SHOPLING_LIVE_PIPELINE_VERSION,
          phase: externalWriteStarted ? "start_uncertain" : "start_failed",
          error: message,
          externalWriteExecuted: externalWriteStarted,
          externalRequestId,
          updatedAt: new Date().toISOString(),
        },
      }).catch(() => null);
    ...
    return Response.json(
      {
        ok: false,
        code: externalWriteStarted ? "SEO_SHOPLING_LIVE_START_UNCERTAIN" : "SEO_SHOPLING_LIVE_START_FAILED",
        message: externalWriteStarted
          ? `외부 실제등록은 시작됐지만 OPS 상태 저장이 완전히 끝나지 않았습니다. 재실행하지 마세요. 출고센터에서 상태를 확인하세요. ${message}`
          : message,
        dispatchId: dispatchCreated ? dispatchId : undefined,
      },
      { status: externalWriteStarted ? 202 : 400 },
    );
  }
}
