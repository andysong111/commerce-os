...
import {
  dispatchPreparedSeoShoplingProductUpload,
  prepareSeoShoplingProductUpload,
} from "@/lib/seoShoplingProductUploadPrepared";
...
function definitelyNotAccepted(error: unknown) {
  return Boolean(
    error &&
      typeof error === "object" &&
      "definitelyNotAccepted" in error &&
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (Number(detail.stats.review_count) > 0) {
      return Response.json(
        { ok: false, code: "SEO_TITLE_REVIEW_PENDING", message: "이전 실제등록에서 확인이 필요한 상품명이 남아 있습니다. 먼저 확인필요 건을 정리하세요." },
        { status: 409 },
      );
    }

    const recent = await listSeoTitleDispatches(context, { ledgerId, limit: 20 });
    const active = recent.find((row) => ["reserved", "ready", "submitted"].includes(text(row.status)));
    if (active) {
      return Response.json(
        { ok: false, code: "SEO_TITLE_ACTIVE_DISPATCH_EXISTS", message: "이 상품에 이미 진행 중인 실제등록이 있습니다. 완료 후 다음 등록을 실행하세요.", dispatchId: active.dispatch_id },
        { status: 409 },
      );
    }
    ...
    if (canonicalMode === "empty" && usedCount > 0) {
      return Response.json(
        { ok: false, code: "SEO_TITLE_CANONICAL_STATE_MISMATCH", message: "상품명 사용 이력은 있지만 기준 goods_key가 없습니다. 원장 상태 확인이 필요합니다." },
        { status: 409 },
      );
    }
    const mode =
    ...
    const reserved = await callSeoTitleRpc<ReservedTitle[]>(context, "reserve_seo_title_inventory", {
    ...
    const plan = buildReservedPlan(
      ...
      reserved,
      mode === "apply_existing_first" ? canonicalGoodsKeys : undefined,
    );
    ...
    const dispatch = await insertSeoTitleDispatch(context, {
      ...
      reservation_expires_at: reserved[0]?.reservation_expires_at ?? null,
      submitted_at: now,
    });
    ...
    if (mode === "apply_existing_first") {
      ...
      const prepared = prepareSeoShoplingDirectApply(plan, canonicalGoodsKeys);
      externalRequestId = prepared.requestId;
      externalPhase = "direct_apply_dispatching";
    ...
    const prepared = await prepareSeoShoplingProductUpload(context, {
    ...
    externalRequestId = prepared.requestId;
    externalPhase = "base_upload_dispatching";
    await patchSeoTitleDispatch(context, dispatchId, {
      external_request_id: prepared.requestId,
      result_payload: {
        pipelineVersion: SEO_SHOPLING_LIVE_PIPELINE_VERSION,
        ...
        mode,
        canonicalSeed: mode === "canonical_seed",
        productUploadJobId: prepared.jobId,
        productUploadRequestId: prepared.requestId,
        productUploadActionsUrl: prepared.actionsUrl,
        externalWriteExecuted: false,
        updatedAt: new Date().toISOString(),
      },
    });
    handoffAttempted = true;
    const upload = await dispatchPreparedSeoShoplingProductUpload(prepared);
    await patchSeoTitleDispatch(context, dispatchId, {
      result_payload: {
        pipelineVersion: SEO_SHOPLING_LIVE_PIPELINE_VERSION,
    ...
    const message = error instanceof Error ? error.message : "샵플링 실제등록을 시작하지 못했습니다.";
    const explicitReject = definitelyNotAccepted(error);
    const uncertain = handoffAttempted && !explicitReject;

    if (reservationCreated && !uncertain) {
    ...
    if (dispatchCreated) {
      await patchSeoTitleDispatch(context, dispatchId, {
        status: uncertain ? "submitted" : "cancelled",
        completed_at: uncertain ? null : new Date().toISOString(),
        ...(externalRequestId ? { external_request_id: externalRequestId } : {}),
        result_payload: {
          pipelineVersion: SEO_SHOPLING_LIVE_PIPELINE_VERSION,
          phase: uncertain ? "start_uncertain" : "start_failed",
          previousPhase: externalPhase,
          error: message,
          externalWriteExecuted: uncertain,
          externalRequestId,
          updatedAt: new Date().toISOString(),
        },
      }).catch(() => null);
    ...
    return Response.json(
      {
        ok: false,
        code: uncertain ? "SEO_SHOPLING_LIVE_START_UNCERTAIN" : "SEO_SHOPLING_LIVE_START_FAILED",
        message: uncertain
          ? `외부 실제등록 요청 결과가 불확실합니다. 재실행하지 마세요. 출고 실행기에서 상태를 확인하세요. ${message}`
          : message,
        dispatchId: dispatchCreated ? dispatchId : undefined,
      },
      { status: uncertain ? 202 : 400 },
    );
  }
}
