...
function sourceFrom(value: unknown): KeywordElonSourceDraft {
  ...
  return {
    ...
    autoStatus:
      value.autoStatus === "success" ||
      value.autoStatus === "partial" ||
      value.autoStatus === "failed"
        ? value.autoStatus
        : "idle",
    chineseTitle: text(value.chineseTitle),
    optionText: text(value.optionText),
    supportingText: text(value.supportingText),
    warnings: textArray(value.warnings, 20),
...
function identityFrom(value: unknown): KeywordElonIdentity {
  ...
  return {
    koreanProductIdentity: text(value.koreanProductIdentity),
    coreProduct: text(value.coreProduct),
    identityAnchor: text(value.identityAnchor),
    primarySeeds: strings("primarySeeds"),
    conditionalSeeds: strings("conditionalSeeds"),
    functionModifiers: strings("functionModifiers"),
    designShapeModifiers: strings("designShapeModifiers"),
    specAttributes: strings("specAttributes"),
    variantNoise: strings("variantNoise"),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "collect_source") {
      ...
      if (!url) throw new Error("1688 링크를 입력해 주세요.");
      return NextResponse.json({
        ok: true,
        action,
        source: await collectKeywordElon1688Source(url),
      });
    }
    if (action === "analyze_identity") {
      return NextResponse.json({
        ok: true,
        action,
        identity: await analyzeKeywordElonIdentity(sourceFrom(body.source)),
      });
    }
    if (action === "discover_keywords") {
      ...
      return NextResponse.json({
        ok: true,
        action,
        discovery: await discoverKeywordElonCandidatesResilient(source, identity),
      });
    }
    ...
    if (action === "generate_title") {
      ...
      const cutoff = Math.max(
        0,
        Math.min(100, Number.isFinite(rawCutoff) ? rawCutoff : 70),
      );
    ...
    return NextResponse.json(
      { ok: false, errorStage: action, error: `지원하지 않는 action: ${action}` },
      { status: 400 },
    );
  } catch (error) {
    const message =
