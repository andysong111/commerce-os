...
export async function GET() {
  ...
  for (const fileName of FILES) {
    ...
    if (fileName === "manifest.json") {
      ...
      if (manifest.background?.service_worker !== "background-v039.js") throw new Error("shopling_a21_resend_background_v039_required");
      ...
      if (!mainRuntime || mainRuntime.js?.some((name) => name.includes("result-bridge"))) {
        throw new Error("shopling_a21_resend_v039_native_submit_required");
      }
      if (manifest.content_scripts?.some((item) => item.js?.some((name) => /result-(loading|bridge|relay|complete|wait)/.test(name)))) {
        throw new Error("shopling_a21_resend_v039_result_runtime_forbidden");
      }
