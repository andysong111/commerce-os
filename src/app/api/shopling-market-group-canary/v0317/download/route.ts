...
function rewriteContent(source: string) {
  ...
  const oldDriveStart = `  async function drive() {\n    if (driving) return;\n    driving = true;\n    try {\n      const context = await workerContext();`;
  const newDriveStart = `  async function drive() {\n    if (driving) return;\n    driving = true;\n    try {\n      if (isMallResultFrame()) broadcastMallResultEvidence();\n      const context = await workerContext();`;
  rewritten = replaceOnce(rewritten, oldDriveStart, newDriveStart, "v0317_content_drive_broadcast_missing");
