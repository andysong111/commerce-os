...
export async function GET() {
  ...
  entries["VERSION.txt"] = strToU8("Commerce OS Shopling Market Canary v0.1.0\n");
  const archive = zipSync(entries, { level: 6 });
  return new Response(Buffer.from(archive), {
    status: 200,
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": "attachment; filename=commerce-os-shopling-market-canary-v0.1.0.zip",
