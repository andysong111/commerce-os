...
export async function POST(_request:Request,{params}:{params:Promise<{jobId:string}>}) {
 ...
 const manual=await requireManualShoplingPriceBulkJob(auth.admin!,jobId,auth.ownerId,"normal.result");if(manual.response)return manual.response;
 if(!["normal_running","dispatch_uncertain"].includes(String(manual.job.status)))return normalError("현재 상태에서는 일반 청크 결과를 확인할 수 없습니다.",409,"INVALID_JOB_STATUS","normal.result.job_query");
 const chunkResult=await auth.admin!.from("shopling_price_bulk_chunks").select("chunk_index,goods_keys,goods_key_count,request_id,status,started_at,updated_at").eq("job_id",jobId).eq("chunk_type","normal").in("status",["dispatching","running","dispatch_uncertain"]).order("chunk_index",{ascending:true}).limit(2);
