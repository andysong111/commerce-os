...
export async function GET(request: Request) {
  if (process.env.VERCEL_ENV !== "production") {
    return NextResponse.json({ ok: false, message: "SEO 실제등록 후처리는 Production에서만 실행됩니다." }, { status: 403 });
  }
  const secret = process.env.CRON_SECRET?.trim();
  if (!secret) {
    return NextResponse.json({ ok: false, message: "CRON_SECRET 설정이 없어 후처리를 차단했습니다." }, { status: 503 });
  }
  if (!authorized(request, secret)) {
    return NextResponse.json({ ok: false, message: "SEO 실제등록 후처리 인증에 실패했습니다." }, { status: 401 });
  }
  const config = getProductLaunchAdminConfig();
  if (!config.ok) return NextResponse.json(config.body, { status: config.status });

  const dispatches = await listActiveSeoLiveDispatches(config.value, MAX_ACTIVE_DISPATCHES);
  ...
  for (const dispatch of dispatches) {
    ...
    const requestId = text(resultPayload.directApplyRequestId || dispatch.external_request_id);
    if (!dispatchId || !ownerId || !reservationId || !requestId) continue;
    if (!["direct_apply_queued", "direct_apply_running"].includes(phase)) continue;
    ...
    try {
      ...
      if (applied.status === "pending") {
        await patchSeoLiveDispatchByOwner(config.value, ownerId, dispatchId, {
          result_payload: {
            ...
            directApplyRunUrl: applied.runUrl || resultPayload.directApplyRunUrl || "",
            directApplyMessage: applied.message || "실제 반영 진행 중",
      ...
      if (verified) {
        await callSeoLiveRpc<number>(config.value, "finalize_seo_title_reservation", {
          p_owner_id: ownerId,
      ...
      if (applied.status === "error" && applied.phase === "unknown") {
        await patchSeoLiveDispatchByOwner(config.value, ownerId, dispatchId, {
          result_payload: {
            ...resultPayload,
            phase,
            reconcileWarning: applied.message || "결과 확인 중 일시 오류",
            updatedAt: new Date().toISOString(),
          },
        });
      ...
      await Promise.all([
        patchSeoLiveDispatchItemsByOwner(config.value, ownerId, dispatchId, {
          status: "failed",
          error_message: applied.message || "상품명·검색어 실제 반영 결과를 확인해야 합니다.",
        }),
        patchSeoLiveDispatchByOwner(config.value, ownerId, dispatchId, {
          status: "failed",
          completed_at: completedAt,
          result_payload: {
            ...
            directApplyRunUrl: applied.runUrl || resultPayload.directApplyRunUrl || "",
            directApplySummary: applied.summary || {},
            directApplyMessage: applied.message || "실제 반영 실패 또는 부분 실패",
            externalWriteExecuted: true,
            updatedAt: completedAt,
          },
        }),
      ...
      results.push({
        dispatchId,
        status: "retry",
        message: error instanceof Error ? error.message : "결과 확인 실패",
      });
    }
  }
