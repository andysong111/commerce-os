...
export default function ShoplingSeoDispatchCenter() {
    ...
    try {
      const result = await requestJson<ReserveResponse>("/api/seo-title-dispatch", {
        ...
        body: JSON.stringify({
          ...
          rounds: registrationRounds,
        }),
      });
      if (!result.plan) throw new Error(result.message || "출고 계획을 만들지 못했습니다.");
      setActivePlan(result.plan);
      setMessage(
        `${ledger.model_number || ledger.model_name}: ${result.plan.reservedTitleCount}개 상품명을 예약하고 샵플링 실행계획을 만들었습니다.`,
      );
      await load();
    } catch (caught) {
  ...
  async function release(dispatchId: string) {
    ...
    try {
      const result = await requestJson<{ ok?: boolean; releasedCount?: number; message?: string }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ action: "release", dispatchId }),
        },
      );
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-violet-700">
              COMMERCE OS · SHOPLING SEO DISPATCH
            </p>
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-3 py-1 text-xs font-black ${lowStock ? "bg-amber-100 text-amber-900" : "bg-emerald-100 text-emerald-800"}`}>
                        전체몰 {number(ledger.full_market_rounds_available)}회분
                      </span>
                ...
                <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 lg:flex-row lg:items-end">
                  <label className="text-sm font-black">
                    ...
                    <select
                      ...
                      onChange={(event) => setRounds((current) => ({ ...current, [ledger.ledger_id]: Number(event.target.value) }))}
                      className="mt-2 block rounded-xl border border-slate-300 bg-white px-4 py-3"
                    >
                      {Array.from({ length: 20 }, (_, index) => index + 1).map((value) => (
                        <option key={value} value={value}>{value}회 · 상품명 {value * 29}개</option>
                      ))}
                    </select>
                  </label>
                  <div className="flex-1 text-xs leading-5 text-slate-600">
                    전송 전 예상 잔량: <strong>{Math.max(0, number(ledger.available_count) - selectedRounds * 29).toLocaleString()}개</strong>
                    <br />
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => void reserve(ledger)}
                      disabled={busyLedgerId === ledger.ledger_id || ledger.full_market_rounds_available < selectedRounds}
                    ...
                    >
                      {busyLedgerId === ledger.ledger_id ? "예약 중" : "출고 계획 예약"}
                    </button>
                  </div>
                </div>
              </article>
      ...
      {activePlan ? (
        <section className="rounded-3xl border-2 border-violet-200 bg-violet-50/50 p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <button
              type="button"
              onClick={() => void release(activePlan.dispatchId)}
              disabled={busyDispatchId === activePlan.dispatchId}
              className="rounded-xl border border-rose-300 bg-white px-4 py-2 text-sm font-black text-rose-700 disabled:opacity-50"
            >
              {busyDispatchId === activePlan.dispatchId ? "해제 중" : "예약 취소·재고 복귀"}
            </button>
          </div>
          <div className="mt-4 overflow-x-auto rounded-2xl border border-violet-100 bg-white">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-violet-50 text-xs text-violet-900">
                <tr>
                  <th className="px-4 py-3">회차</th>
                  <th className="px-4 py-3">상품그룹</th>
                  <th className="px-4 py-3">쇼핑몰</th>
                  <th className="px-4 py-3">goods_key</th>
                  <th className="min-w-[420px] px-4 py-3">예약 상품명</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {activePlan.items.map((item) => (
                  <tr key={`${item.registration_round}:${item.product_group}:${item.mall_key}:${item.account_id_label}`}>
          ...
          <div className="mt-4 rounded-xl bg-white px-4 py-3 text-sm font-bold text-slate-700 ring-1 ring-violet-100">
            외부 샵플링 전송은 비활성화 상태입니다. 반복 신규등록용 API 경로가 검증되면 이 예약 계획을 그대로 제출 단계에 연결합니다.
          </div>
        </section>
      ) : null}
          ...
          <div className="mt-4 space-y-2">
            {dispatches.map((dispatch) => (
              <div key={dispatch.dispatch_id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm">
                <div>
                  <div className="font-black">{dispatch.registration_rounds}회분 · 상품명 {dispatch.reserved_title_count}개 · {dispatch.status}</div>
                  <div className="mt-1 text-xs text-slate-500">생성 {dateText(dispatch.created_at)} · 예약 만료 {dateText(dispatch.reservation_expires_at)}</div>
                </div>
                {dispatch.status === "reserved" || dispatch.status === "ready" ? (
                  <button type="button" onClick={() => void release(dispatch.dispatch_id)} disabled={busyDispatchId === dispatch.dispatch_id} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-black text-rose-700 disabled:opacity-50">
                    예약 해제
                  </button>
                ) : null}
              </div>
