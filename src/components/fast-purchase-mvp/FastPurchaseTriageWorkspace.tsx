...
export function FastPurchaseTriageWorkspace({
  ...
  const summary = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      const quantity = isSystemOrder(row.action)
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (filter === "ORDER") {
        return isSystemOrder(row.action) || entry.plannedQuantity > 0;
      }
      if (filter === "MANUAL") return isManual(row.action);
      if (filter === "PENDING") return isManual(row.action) && entry.stockSense === "UNKNOWN";
      if (filter === "PLANNED") {
        return (isSystemOrder(row.action) ? row.recommendedQuantity : entry.plannedQuantity) > 0;
      }
      return true;
    });
  ...
  const exportPlan = () => {
    const planRows = rows.flatMap((row) => {
      const entry = triage[row.barcode] ?? EMPTY_ENTRY;
      const plannedQuantity = isSystemOrder(row.action)
        ? row.recommendedQuantity
        : entry.plannedQuantity;
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-4xl text-sm leading-6 text-slate-500">
            수동검토 상품은 창고를 전수조사하지 말고 기억·현장 체감으로 `충분 / 부족 / 품절`만 빠르게 표시하세요. 주문 예정수량은 직접 확정합니다. 판단과 메모는 이 브라우저에만 저장되며 Commerce OS 서버, Product Master, 중국 주문에는 쓰지 않습니다.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={exportPlan} className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white hover:bg-slate-800">
            주문예정 CSV
          </button>
          <button type="button" onClick={clearTriage} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
            브라우저 판단 초기화
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {([
          ...
          <button
            ...
            className={`rounded-xl border px-3 py-2 text-sm font-bold ${filter === value ? "border-slate-950 bg-slate-950 text-white" : "border-slate-300 bg-white text-slate-700"}`}
          >
            {label}
          </button>
        ))}
                ...
                <tr key={row.barcode} className="align-top">
                    ...
                    {manual ? (
                      <div className="flex min-w-[300px] flex-wrap gap-1.5">
                        {([
                          ...
                          ["OUT", "품절"],
                          ["UNKNOWN", "모름"],
                        ] as Array<[StockSense, string]>).map(([value, label]) => (
                          <button
                            key={value}
                            type="button"
                            onClick={() => updateEntry(row.barcode, {
                              stockSense: value,
                              ...(value === "ENOUGH" || value === "UNKNOWN" ? { plannedQuantity: 0 } : {}),
                            })}
                            className={`rounded-lg border px-2.5 py-1.5 text-xs font-bold ${entry.stockSense === value ? "border-slate-950 bg-slate-950 text-white" : "border-slate-300 bg-white text-slate-700"}`}
                          >
                            {label}
                          </button>
                        ))}
                      </div>
                    ) : <span className="text-xs text-slate-400">시스템판정</span>}
                  </td>
                  <td className="px-3 py-4 text-right">
                    {isSystemOrder(row.action) ? (
                      <strong className="text-lg text-blue-700">{number.format(row.recommendedQuantity)}</strong>
                    ) : manual ? (
                      <div className="flex min-w-[190px] items-center justify-end gap-2">
                        <input
                          ...
                          min={0}
                          step={1}
                          value={entry.plannedQuantity || ""}
                          onChange={(event) => updateEntry(row.barcode, { plannedQuantity: Math.max(0, Math.round(Number(event.target.value) || 0)) })}
                          placeholder="직접 입력"
                          className="w-24 rounded-lg border border-slate-300 px-2 py-1.5 text-right font-black outline-none focus:border-slate-500"
                        />
                        {(entry.stockSense === "LOW" || entry.stockSense === "OUT") && row.referenceDemandQuantity > 0 ? (
                          <button
                            type="button"
                            onClick={() => updateEntry(row.barcode, { plannedQuantity: row.referenceDemandQuantity })}
                  ...
                  <td className="px-3 py-4">
                    {manual ? (
                      <input
                        value={entry.note}
                        onChange={(event) => updateEntry(row.barcode, { note: event.target.value.slice(0, 300) })}
                  ...
                  <td className="max-w-[420px] px-3 py-4 text-xs leading-5 text-slate-500">
                    <span className="font-mono font-black text-slate-700">{row.basis}</span><br />
                    {row.reason}
                  </td>
                </tr>
              );
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-950">
        `참고상한 넣기`는 수량 입력을 돕는 버튼일 뿐 주문 실행이 아닙니다. 재고 근거 없는 상품에서 참고상한을 그대로 주문하면 과잉발주가 될 수 있으므로, `부족/품절`이라고 실제로 판단한 경우에만 사용하세요. 저장 범위는 현재 브라우저 localStorage뿐입니다.
      </div>
