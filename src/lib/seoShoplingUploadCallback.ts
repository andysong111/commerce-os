...
export async function handleSeoShoplingProductUploadCallback(
  ...
  if (items.length !== 29) {
    await failDispatch(context, meta, "예약된 전체몰 상품명이 29개가 아니어서 실제 반영을 중단했습니다.", completedAt);
  ...
  try {
    ...
    const apply = await dispatchSeoShoplingDirectApply(refreshedItems, goodsKeys);
