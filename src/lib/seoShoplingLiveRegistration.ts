...
export const SEO_SHOPLING_GROUPS = ["도매1", "도매2", "도매3", "도매4", "소매1", "소매2"] as const;
...
const GROUP_BY_LABEL = Object.fromEntries(
  SEO_SHOPLING_GROUPS.map((group) => [group, group]),
) as Record<string, SeoShoplingGroup>;
...
export function emptySeoShoplingGoodsKeys(): SeoShoplingGoodsKeys {
  return { 도매1: "", 도매2: "", 도매3: "", 도매4: "", 소매1: "", 소매2: "" };
}
...
export function extractSeoShoplingGoodsKeys(rowsInput: unknown): SeoShoplingGoodsKeys {
  ...
  for (const value of rows) {
    ...
    const group =
      GROUP_BY_CHANNEL_KEY[text(row.channel_key ?? row.channelKey)] ||
      GROUP_BY_LABEL[text(row.channel ?? row.channel_label)];
...
export function decorateSeoShoplingBulkPayload(
  ...
  metadata: { dispatchId: string; ledgerId: string; reservationId: string; canonicalSeed: boolean },
) {
...
export function buildSeoShoplingDirectPlan(itemsInput: unknown, goodsKeys: SeoShoplingGoodsKeys) {
  ...
  const plan = items.map((value) => {
    ...
    return { goods_key: goodsKey, mall_key: mallKey, final_title: finalTitle, final_site_srch: finalSiteSrch };
  });
...
export function seoShoplingDirectApplySucceeded(
  result: { status?: unknown; summary?: Record<string, unknown> },
  expectedItems = 29,
) {
...
export async function readSeoShoplingLaunchState(context: SeoTitleLedgerContext, launchItemId: string) {
...
export async function createSeoShoplingProductUploadJob(
  ...
  const response = await fetch(`${context.config.supabaseUrl}/rest/v1/product_launch_upload_jobs`, {
    method: "POST",
    headers: { ...createSupabaseAdminHeaders(context.config.secretKey), Prefer: "return=representation" },
  ...
  return { jobId, requestId, actionsUrl: dispatch.actionsUrl, payload };
}
