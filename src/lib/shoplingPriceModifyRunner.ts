...
export function buildShoplingPriceModifyDispatchRequest(goodsKeyInput: string, policyOverridesInput?: unknown, goodsKeyGroupJsonInput?: unknown, basePriceInput?: { base_consumer_price?: unknown; base_sell_price?: unknown; base_purchase_price?: unknown; base_prices_json?: unknown }, reservedRequestId?: string) {
  ...
  if (!isValidShoplingPriceModifyRequestId(requestId)) throw new Error("요청 추적 ID 형식이 올바르지 않습니다.");
  return {
...
export async function dispatchReservedShoplingPriceModifyActions(goodsKeyInput: string, policyOverridesInput: unknown, requestId: string) {
  const request = buildShoplingPriceModifyDispatchRequest(goodsKeyInput, policyOverridesInput, undefined, undefined, requestId);
  ...
  if (response.status !== 204 && response.status !== 200) throw new Error(`GitHub Actions 워크플로 실행 요청에 실패했습니다. status=${response.status}`);
  return { requestId, githubActionsUrl: request.githubActionsUrl };
}
