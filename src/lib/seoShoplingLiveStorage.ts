...
async function request<T>(config: ProductLaunchAdminConfig, path: string, init: RequestInit = {}) {
...
export async function listActiveSeoLiveDispatches(config: ProductLaunchAdminConfig, limit = 5) {
  const params = new URLSearchParams({
    select: "*",
    status: "eq.submitted",
...
export async function patchSeoLiveDispatchByOwner(
  ...
  const params = new URLSearchParams({ owner_id: `eq.${ownerId}`, dispatch_id: `eq.${dispatchId}` });
  const rows = await request<Array<Record<string, unknown>>(
    config,
    `seo_title_dispatches?${params.toString()}`,
