...
function listingGoodsKeys(product: { listings?: Array<{ goodsKey?: string | null; active?: boolean }> }) {
  return [...new Set((product.listings ?? []).filter((listing) => listing.active !== false).map((listing) => text(listing.goodsKey)).filter((value) => /^\d{5,9}$/.test(value)))];
}
...
export async function runProductLifecycleLiveReconcile(nowValue: Date | string = new Date()): Promise<ProductLifecycleLiveReconcileSummary> {
  ...
  const [planning, statesResult, blockingQueueResult] = await Promise.all([
    ...
    admin.from(STATE_TABLE).select("sku_id,barcode,lifecycle_state,desired_shopling_state,requires_review,reason_codes,evidence").limit(MAX_PRODUCTS),
    admin.from(QUEUE_TABLE).select("goods_key,desired_state,status").eq("shadow_mode", false).in("status", ["pending", "claimed", "failed", "confirm_needed"]).limit(MAX_PRODUCTS),
  ]);
  if (planning.products.length > MAX_PRODUCTS) throw new Error(`PRODUCT_LIFECYCLE_RECONCILE_PRODUCT_LIMIT_EXCEEDED:${planning.products.length}`);
  ...
  const blockingGoodsKeys = new Set((Array.isArray(blockingQueueResult.data) ? blockingQueueResult.data : []).map((row) => text(row.goods_key)).filter((value) => /^\d{5,9}$/.test(value)));
  const eligibleProducts = planning.products.map((product) => ({ product, state: statesBySku.get(text(product.skuId)) })).filter(({ state }) => state && state.requires_review !== true && Boolean(desiredSaleStatus(state.desired_shopling_state)));
  ...
  for (const { product, state } of eligibleProducts) {
    ...
    for (const goodsKey of listingGoodsKeys(product)) {
      ...
      mismatchRows.push({
        dedupe_key: ["live-reconcile", text(state.sku_id), goodsKey, current, desiredState, currentHourKey(generatedAt)].join(":"),
        ...
        evidence: { ...stateEvidence, runtimeReconcile: true, runtimeConfigSource: config.source, observedShoplingSaleStatus: current, desiredShoplingSaleStatus: desiredCode, reconciledAt: generatedAt, deleteExecutionAllowed: false },
  ...
  if (config.shoplingNonDestructiveLive && liveRows.length) {
    const queued = await admin.from(QUEUE_TABLE).upsert(liveRows, { onConflict: "dedupe_key", ignoreDuplicates: true });
    if (queued.error) throw new Error(`PRODUCT_LIFECYCLE_RECONCILE_QUEUE_FAILED:${queued.error.message}`);
  ...
  const summary: ProductLifecycleLiveReconcileSummary = {
    ...
    queuedMismatchCount,
    deferredMismatchCount: config.shoplingNonDestructiveLive ? Math.max(0, mismatchRows.length - liveRows.length) : mismatchRows.length,
    unresolvedCount,
...
async function storeOperation(admin: AdminClient, summary: ProductLifecycleLiveReconcileSummary) {
  ...
  if (result.error) throw new Error(`PRODUCT_LIFECYCLE_RECONCILE_OPERATION_STORE_FAILED:${result.error.message}`);
}
