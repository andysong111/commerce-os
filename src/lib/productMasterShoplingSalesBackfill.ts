...
function mappingFingerprint(products: Awaited<ReturnType<typeof loadProductPlanningSnapshot>>["products"]) {
...
async function storeOperation(input: StoreOperationInput) {
  ...
  if (!response.ok) {
    throw new Error(`PRODUCT_MASTER_SHOPLING_SALES_STORE_FAILED:${response.status}:${body.slice(0, 300)}`);
  }
  return body ? JSON.parse(body) : [];
}

async function readOperations(operationType: string, requestCorrelationId?: string, limit = OPERATION_LIMIT) {
  const admin = await createSupabaseAdminClient();
  if (!admin) throw new Error("SUPABASE_ADMIN_NOT_CONFIGURED");
  let query = admin
    .from("commerce_operation_runs")
    .select("operation_type,source_event_id,correlation_id,status,input_snapshot,result_snapshot,error_message,started_at")
...
function recentStart(now: Date) {
  return dateOnly(
    new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - (DEFAULT_MONTHS - 1), 1)),
  );
}
...
export async function createProductMasterShoplingSalesRequest(options: {
  ...
  const chunkDays = Math.max(1, Math.min(31, Math.round(options.chunkDays ?? PRODUCT_MASTER_SHOPLING_SALES_DEFAULT_CHUNK_DAYS)));
  const request: SalesBackfillRequest = {
    ...
    ranges: splitShoplingDateRange(recentStart(now), dateOnly(now), chunkDays),
...
function requestFromRow(row: OperationRow): SalesBackfillRequest | null {
  ...
  if (!requestId || !createdAt || !ranges.length || !/^sha256:[a-f0-9]{64}$/.test(fingerprint)) {
    return null;
  }
  return {
    requestId,
...
export async function runProductMasterShoplingSalesStep() {
  ...
  if (!context) return { processed: false, state: "IDLE", message: "판매원장 작업이 없습니다." };
  if (context.reports.some(reportFromRow)) {
    return { processed: false, state: "COMPLETED", message: "판매원장 읽기 진단이 이미 완료되었습니다." };
  }
  if (context.failedRuns.length) {
    return { processed: false, state: "FAILED", message: "판매원장 읽기 진단이 실패 종료되었습니다." };
  }
  const chunks = context.chunks.map(chunkFromRow).filter(Boolean) as ProductMasterShoplingSalesChunk[];
  const completed = new Set(chunks.map((chunk) => rangeKey(chunk.range)));
  const nextRange = context.request.ranges.find((range) => !completed.has(rangeKey(range)));

  if (nextRange) {
    const key = rangeKey(nextRange);
    const attempts = context.failures.map(failureAttempt).filter((failure) => failure.rangeKey === key);
    if (attempts.length >= MAX_STEP_ATTEMPTS) {
      ...
      await storeOperation({
        ...
        status: "FAILED",
        inputSnapshot: { requestId: context.request.requestId, rangeKey: key, chunkDays: context.request.chunkDays },
    ...
    try {
      ...
      const result = aggregateProductMasterShoplingSalesChunk(rows, planning, nextRange);
      await storeOperation({
        ...
        correlationId: context.cid,
        inputSnapshot: { requestId: context.request.requestId, range: nextRange, rangeKey: key, chunkDays: context.request.chunkDays },
      ...
      await storeOperation({
        ...
        correlationId: context.cid,
        status: "FAILED",
        inputSnapshot: { requestId: context.request.requestId, range: nextRange, rangeKey: key, chunkDays: context.request.chunkDays, attempt },
        resultSnapshot: { state: "RETRY_PENDING", message },
        errorMessage: message,
      });
      return { processed: false, state: "RUNNING", retryPending: true, attempt, range: nextRange, message };
    }
  }

  const planning = await loadProductPlanningSnapshot();
  if (mappingFingerprint(planning.products) !== context.request.mappingFingerprint) {
    const message = "판매원장 수집 중 상품마스터 Shopling 연결구조가 변경되어 서로 다른 기준을 섞지 않도록 종료했습니다.";
    await storeOperation({
      operationType: PRODUCT_MASTER_SHOPLING_SALES_FAILED,
      sourceEventId: `product-master-shopling-sales-failed:${context.request.requestId}`,
      correlationId: context.cid,
  ...
  await storeOperation({
    ...
    correlationId: context.cid,
    inputSnapshot: { requestId: context.request.requestId, rangeCount: context.request.ranges.length, chunkDays: context.request.chunkDays },
    resultSnapshot: { report },
  });
  return { processed: true, state: "COMPLETED", report, message: "최근 24개월 Shopling 주문 읽기와 월 판매량 후보 계산을 완료했습니다." };
}

async function loadProductMasterSalesSnapshot(): Promise<SalesSnapshot> {
  const { baseUrl, secret } = productMasterConnection();
  const response = await fetch(`${baseUrl}/api/integrations/sales-ledger-snapshot`, {
    method: "GET",
  ...
  if (!response.ok || payload.ok !== true || !Array.isArray(payload.rows)) {
    throw new Error(payload.message || `PRODUCT_MASTER_SALES_SNAPSHOT_FAILED:${response.status}`);
  }
  return { rows: payload.rows, sourceCounts: payload.sourceCounts ?? {} };
}

function exactSales(left: ProductMasterSalesMonthlyRow, right: SalesSnapshotRow) {
  return (
    left.id === right.id &&
...
function buildApplyPlan(
  ...
  const skuByBarcode = new Map(
    planning.products
      .filter((product) => product.skuActive !== false)
      .map((product) => [text(product.barcode).toUpperCase(), text(product.skuId)]),
  );
  ...
  for (const row of rows) {
    const skuId = skuByBarcode.get(row.barcode);
    if (!skuId) {
      blockers.push({ code: "SKU_NOT_CURRENT", barcode: row.barcode, skuId: null, month: row.month, message: "판매원장 위치코드를 현재 상품마스터 SKU에서 찾지 못했습니다." });
      continue;
    }
    const resolved = { ...row, skuId };
    ...
    if (current) {
      if (current.skuId === skuId && exactSales(row, current)) {
        ...
        blockers.push({ code: "TARGET_ROW_CONFLICT", barcode: row.barcode, skuId, month: row.month, message: "같은 Shopling 판매원장 ID에 다른 값이 이미 있어 자동 덮어쓰기를 차단했습니다." });
      }
      continue;
    }
    const overlaps = otherBySkuMonth.get(`${skuId}\u0000${row.month}`) ?? [];
    if (overlaps.some((existingRow) => existingRow.quantity > 0 || existingRow.revenue > 0)) {
      blockers.push({ code: "LEGACY_MONTH_OVERLAP", barcode: row.barcode, skuId, month: row.month, message: "같은 SKU·월에 다른 원천의 판매원장이 있어 이중계상 위험 때문에 자동 적재를 차단했습니다." });
      continue;
    }
    safeRows.push(resolved);
    pending.push(resolved);
  }
  return { totalRows: rows.length, safeRows, alreadyApplied, pending, blockers };
}

async function combinedRows(context: NonNullable<Awaited<ReturnType<typeof activeContext>>>) {
  const chunks = context.chunks.map(chunkFromRow).filter(Boolean) as ProductMasterShoplingSalesChunk[];
  return combineProductMasterShoplingSalesChunks(chunks).rows;
}

function canaryVerified(context: NonNullable<Awaited<ReturnType<typeof activeContext>>>) {
  return context.canaries.some((row) => text(row.status) === "SUCCEEDED" && object(row.result_snapshot).verified === true);
}
...
async function pushSales(rows: ApplyRow[]) {
  ...
  if (!response.ok || payload.ok !== true) {
    throw new Error(payload.message || payload.error || `PRODUCT_MASTER_SALES_WRITE_FAILED:${response.status}`);
  }
}

async function currentApplyPlan(context: NonNullable<Awaited<ReturnType<typeof activeContext>>>) {
  const planning = await loadProductPlanningSnapshot();
  if (mappingFingerprint(planning.products) !== context.request.mappingFingerprint) {
    throw new Error("PRODUCT_MASTER_SALES_MAPPING_CHANGED");
  }
  const [rows, existing] = await Promise.all([combinedRows(context), loadProductMasterSalesSnapshot()]);
  return buildApplyPlan(rows, planning, existing);
}

export async function applyProductMasterShoplingSales(mode: "CANARY" | "FULL") {
  const context = await activeContext();
  if (!context) throw new Error("PRODUCT_MASTER_SALES_DIAGNOSTIC_NOT_COMPLETED");
  ...
  if (report.unmappedRows > 0) throw new Error(`PRODUCT_MASTER_SALES_BLOCKED:unmapped=${report.unmappedRows}`);
  const plan = await currentApplyPlan(context);
  if (plan.blockers.length) throw new Error(`PRODUCT_MASTER_SALES_BLOCKED:conflicts=${plan.blockers.length}`);
  ...
  if (!selected.length) return { mode, applied: 0, verified: true, status: await loadProductMasterShoplingSalesStatus() };

  const operationType = mode === "CANARY" ? PRODUCT_MASTER_SHOPLING_SALES_CANARY : PRODUCT_MASTER_SHOPLING_SALES_FULL;
  const sourceEventId = mode === "CANARY"
  ...
  try {
    ...
    const verifiedIds = new Set(verifiedPlan.alreadyApplied.map((row) => row.id));
    const missing = selected.filter((row) => !verifiedIds.has(row.id));
    if (verifiedPlan.blockers.length || missing.length) {
      throw new Error(`PRODUCT_MASTER_SALES_VERIFY_FAILED:blockers=${verifiedPlan.blockers.length}:missing=${missing.length}`);
    }
    await storeOperation({
      operationType,
      sourceEventId,
      correlationId: context.cid,
      inputSnapshot: { requestId: context.request.requestId, mode, selectedCount: selected.length },
      resultSnapshot: { verified: true, written, pendingCount: verifiedPlan.pending.length, blockerCount: verifiedPlan.blockers.length },
    });
    return {
      mode,
      applied: written,
    ...
    await storeOperation({
      ...
      status: "FAILED",
      inputSnapshot: { requestId: context.request.requestId, mode, selectedCount: selected.length },
      resultSnapshot: { verified: false, writtenBeforeFailure: written, retryIsIdempotent: true },
      errorMessage: message,
    }).catch(() => undefined);
    throw error;
  }
...
export async function loadProductMasterShoplingSalesStatus(): Promise<ProductMasterShoplingSalesStatus> {
  ...
  const common = {
    ...
    progress: totalRanges ? Math.min(100, Math.round((completedRanges / totalRanges) * 100)) : 0,
    completedRanges,
    totalRanges,
    fetchedRows: combined.fetchedRows,
    acceptedRows: combined.acceptedRows,
  ...
  if (context.failedRuns.length) {
    ...
    const error = safeMessage(latest.error_message || object(latest.result_snapshot).message);
    return { ...empty, ...common, state: "FAILED", stage: "수집 실패", message: error, error };
  }
  if (!report) {
    return {
      ...empty,
