import {
  buildShoplingReadRequestXml,
  parseShoplingReadResponse,
  shoplingReadConfigFromEnv,
} from "@/lib/shopling/shoplingReadClient";
import { postShoplingXml } from "@/lib/shopling/shoplingTlsTransport";
import {
  createSupabaseAdminClient,
  createSupabaseAdminHeaders,
} from "@/lib/supabase/admin";

export const PRODUCT_MASTER_SHOPLING_PROBE =
  "PRODUCT_MASTER_SHOPLING_PROBE";

const MANAGED_BARCODE = /^[A-Z]{3}\d+-\d+$/;
const MAX_SAFE_MESSAGE_LENGTH = 500;

type ProbeCategory =
  | "SUCCESS"
  | "CONFIGURATION"
  | "TIMEOUT"
  | "DNS"
  | "TLS"
  | "NETWORK"
  | "HTTP"
  | "SHOPLING_RESPONSE"
  | "PARSE"
  | "UNKNOWN";

export type ProductMasterShoplingProbeResult = {
  probeId: string;
  attemptedAt: string;
  probeDate: string;
  ok: boolean;
  category: ProbeCategory;
  code: string;
  safeMessage: string;
  durationMs: number;
  httpStatus: number | null;
  contentType: string | null;
  responseBytes: number;
  rowCount: number;
  managedBarcodeCount: number;
  evidenceStored: boolean;
  sourceWritesEnabled: false;
};

type OperationRow = {
  result_snapshot?: unknown;
};

function shoplingEnvironment() {
  return {
    SHOPLING_LOGIN_ID: process.env.SHOPLING_LOGIN_ID,
    SHOPLING_COMPANY_ID: process.env.SHOPLING_COMPANY_ID,
    SHOPLING_API_AUTH_KEY: process.env.SHOPLING_API_AUTH_KEY,
    SHOPLING_PRODUCTS_API_URL: process.env.SHOPLING_PRODUCTS_API_URL,
    SHOPLING_ORDERS_API_URL: process.env.SHOPLING_ORDERS_API_URL,
    SHOPLING_CLAIMS_API_URL: process.env.SHOPLING_CLAIMS_API_URL,
  };
}

function koreaDate() {
  return new Date(Date.now() + 9 * 60 * 60 * 1_000)
    .toISOString()
    .slice(0, 10);
}

function validDateOnly(value: string) {
  const parsed = Date.parse(`${value}T00:00:00.000Z`);
  return (
    Number.isFinite(parsed) &&
    new Date(parsed).toISOString().slice(0, 10) === value
  );
}

function asRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === "object" && !Array.isArray(value)
    ? (value as Record<string, unknown>)
    : {};
}

function text(value: unknown) {
  return String(value ?? "").trim();
}

function redact(value: unknown) {
  let message = text(value);
  const secrets = [
    process.env.SHOPLING_API_AUTH_KEY,
    process.env.SHOPLING_LOGIN_ID,
    process.env.SHOPLING_COMPANY_ID,
  ]
    .map((secret) => secret?.trim())
    .filter((secret): secret is string => Boolean(secret));
  for (const secret of secrets) {
    message = message.replaceAll(secret, "[REDACTED]");
  }
  message = message
    .replace(/https?:\/\/[^\s)]+/gi, "[URL]")
    .replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [REDACTED]")
    .replace(
      /(api[_-]?auth[_-]?key|token|secret|password)\s*[:=]\s*[^\s,;]+/gi,
      "$1=[REDACTED]",
    );
  return message.slice(0, MAX_SAFE_MESSAGE_LENGTH);
}

function errorEvidence(error: unknown) {
  const messages: string[] = [];
  const codes: string[] = [];
  const names: string[] = [];
  let current: unknown = error;
  for (let depth = 0; depth < 5 && current; depth += 1) {
    if (current instanceof Error) {
      if (current.message) messages.push(current.message);
      if (current.name) names.push(current.name);
      const record = current as Error & { code?: unknown; cause?: unknown };
      if (record.code) codes.push(text(record.code));
      current = record.cause;
      continue;
    }
    const record = asRecord(current);
    if (record.message) messages.push(text(record.message));
    if (record.name) names.push(text(record.name));
    if (record.code) codes.push(text(record.code));
    current = record.cause;
  }
  const combined = redact(
    [...new Set([...codes, ...names, ...messages])]
      .filter(Boolean)
      .join(" · "),
  );
  return {
    code: codes.find(Boolean) || names.find(Boolean) || "UNKNOWN_ERROR",
    message: combined || "Shopling 상품 연결 확인 중 알 수 없는 오류가 발생했습니다.",
  };
}

function classify(code: string, message: string): ProbeCategory {
  const combined = `${code} ${message}`.toUpperCase();
  if (/SHOPLING_CREDENTIAL_REQUIRED|NOT_CONFIGURED/.test(combined)) {
    return "CONFIGURATION";
  }
  if (/TIMEOUT|ETIMEDOUT|ABORT_ERR|UND_ERR_CONNECT_TIMEOUT/.test(combined)) {
    return "TIMEOUT";
  }
  if (/ENOTFOUND|EAI_AGAIN|DNS/.test(combined)) return "DNS";
  if (/CERT|TLS|SSL|EPROTO|UNABLE_TO_VERIFY|SELF_SIGNED/.test(combined)) {
    return "TLS";
  }
  if (/ECONNRESET|ECONNREFUSED|EPIPE|NETWORK|FETCH FAILED/.test(combined)) {
    return "NETWORK";
  }
  if (/SHOPLING_PRODUCTS_HTTP_|HTTP_\d{3}/.test(combined)) return "HTTP";
  if (/SHOPLING_PRODUCTS_RESPONSE_ERROR/.test(combined)) {
    return "SHOPLING_RESPONSE";
  }
  if (/INVALID_RESPONSE|PARSE|XML/.test(combined)) return "PARSE";
  return "UNKNOWN";
}

function managedBarcodeCount(rows: unknown[]) {
  const managed = new Set<string>();
  for (const value of rows) {
    const row = asRecord(value);
    for (const candidate of [
      row.optBarcode,
      row.optPtnOptCd,
      row.ptn_goods_cd,
      row.barcode,
    ]) {
      const normalized = text(candidate).normalize("NFKC").toUpperCase();
      if (MANAGED_BARCODE.test(normalized)) managed.add(normalized);
    }
  }
  return managed.size;
}

function supabaseConnection() {
  const baseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL?.trim().replace(
    /\/$/,
    "",
  );
  const secret = (
    process.env.SUPABASE_SECRET_KEY ||
    process.env.SUPABASE_SERVICE_ROLE_KEY
  )?.trim();
  if (!baseUrl || !secret) return null;
  return { baseUrl, secret };
}

async function storeProbeEvidence(
  result: ProductMasterShoplingProbeResult,
): Promise<boolean> {
  const runtime = supabaseConnection();
  if (!runtime) return false;
  try {
    const response = await fetch(
      `${runtime.baseUrl}/rest/v1/commerce_operation_runs?select=id`,
      {
        method: "POST",
        headers: {
          ...createSupabaseAdminHeaders(runtime.secret),
          Prefer: "return=minimal",
        },
        body: JSON.stringify([
          {
            operation_type: PRODUCT_MASTER_SHOPLING_PROBE,
            status: result.ok ? "SUCCEEDED" : "FAILED",
            source: "ops-center-product-master-shopling-probe",
            source_event_id: `product-master-shopling-probe:${result.probeId}`,
            correlation_id: `product-master-shopling-probe:${result.probeId}`,
            actor_type: "OPS_OPERATOR",
            input_snapshot: {
              probeDate: result.probeDate,
              resource: "products",
              rangeDays: 1,
            },
            result_snapshot: {
              ...result,
              evidenceStored: true,
            },
            error_message: result.ok ? null : result.safeMessage,
            started_at: result.attemptedAt,
            finished_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          },
        ]),
        cache: "no-store",
        signal: AbortSignal.timeout(20_000),
      },
    );
    return response.ok;
  } catch {
    return false;
  }
}

export async function runProductMasterShoplingProbe(
  requestedDate?: string,
): Promise<ProductMasterShoplingProbeResult> {
  const probeDate = text(requestedDate) || koreaDate();
  if (!validDateOnly(probeDate)) {
    throw new Error("SHOPLING_PROBE_DATE_INVALID");
  }
  const probeId = crypto.randomUUID();
  const attemptedAt = new Date().toISOString();
  const startedAt = Date.now();
  let result: ProductMasterShoplingProbeResult;

  try {
    const config = shoplingReadConfigFromEnv(shoplingEnvironment());
    const xml = buildShoplingReadRequestXml(
      "products",
      config,
      { start: probeDate, end: probeDate },
    );
    const response = await postShoplingXml(config.productsUrl, xml, {
      headers: {
        accept: "application/xml, text/xml",
        "content-type": "application/xml; charset=utf-8",
        "user-agent": "commerce-os-ops-center-shopling-probe/1.0",
      },
      timeoutMs: 30_000,
    });
    const body = await response.text();
    const contentType = response.headers.get("content-type");
    if (!response.ok) {
      const code = `SHOPLING_PRODUCTS_HTTP_${response.status}`;
      result = {
        probeId,
        attemptedAt,
        probeDate,
        ok: false,
        category: "HTTP",
        code,
        safeMessage: code,
        durationMs: Date.now() - startedAt,
        httpStatus: response.status,
        contentType,
        responseBytes: Buffer.byteLength(body, "utf8"),
        rowCount: 0,
        managedBarcodeCount: 0,
        evidenceStored: false,
        sourceWritesEnabled: false,
      };
    } else {
      const rows = parseShoplingReadResponse("products", body);
      result = {
        probeId,
        attemptedAt,
        probeDate,
        ok: true,
        category: "SUCCESS",
        code: "SHOPLING_PRODUCTS_PROBE_OK",
        safeMessage:
          "Shopling 상품 API의 하루 범위 읽기 연결에 성공했습니다.",
        durationMs: Date.now() - startedAt,
        httpStatus: response.status,
        contentType,
        responseBytes: Buffer.byteLength(body, "utf8"),
        rowCount: Array.isArray(rows) ? rows.length : 0,
        managedBarcodeCount: Array.isArray(rows)
          ? managedBarcodeCount(rows)
          : 0,
        evidenceStored: false,
        sourceWritesEnabled: false,
      };
    }
  } catch (error) {
    const evidence = errorEvidence(error);
    result = {
      probeId,
      attemptedAt,
      probeDate,
      ok: false,
      category: classify(evidence.code, evidence.message),
      code: evidence.code,
      safeMessage: evidence.message,
      durationMs: Date.now() - startedAt,
      httpStatus: null,
      contentType: null,
      responseBytes: 0,
      rowCount: 0,
      managedBarcodeCount: 0,
      evidenceStored: false,
      sourceWritesEnabled: false,
    };
  }

  result.evidenceStored = await storeProbeEvidence(result);
  return result;
}

function normalizeStoredResult(
  value: unknown,
): ProductMasterShoplingProbeResult | null {
  const result = asRecord(value) as Partial<ProductMasterShoplingProbeResult>;
  if (
    !text(result.probeId) ||
    !text(result.attemptedAt) ||
    !text(result.probeDate) ||
    typeof result.ok !== "boolean" ||
    result.sourceWritesEnabled !== false
  ) {
    return null;
  }
  return result as ProductMasterShoplingProbeResult;
}

export async function loadLatestProductMasterShoplingProbe() {
  const admin = await createSupabaseAdminClient();
  if (!admin) return null;
  const query = await admin
    .from("commerce_operation_runs")
    .select("result_snapshot")
    .eq("operation_type", PRODUCT_MASTER_SHOPLING_PROBE)
    .order("started_at", { ascending: false })
    .limit(1);
  if (query.error) throw new Error(query.error.message);
  const rows = (Array.isArray(query.data) ? query.data : []) as OperationRow[];
  return normalizeStoredResult(rows[0]?.result_snapshot);
}
