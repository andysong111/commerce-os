...
import { extractSeoShoplingGoodsKeys, SEO_SHOPLING_GROUPS } from "@/lib/seoShoplingLiveRegistration";
import { dispatchPreparedSeoShoplingDirectApply, prepareSeoShoplingDirectApply } from "@/lib/seoShoplingDirectPrepared";
...
export function readSeoBulkMetadata(job: UnknownRecord) {
  const meta = record(record(job.payload).seoBulk);
...
export async function handleSeoShoplingProductUploadCallback(
  ...
  job: UnknownRecord,
  input: { status: "success" | "partial_failure" | "failed"; rows: UnknownRecord[]; errorMessage: string; result?: UnknownRecord },
  completedAt: string,
) {
  const meta = readSeoBulkMetadata(job);
  if (!meta) return { handled: false as const };
  const identity: ProductLaunchIdentity = { userId: text(job.owner_id), email: text(job.owner_email) };
  const context: SeoTitleLedgerContext = { config, identity };
  const items = await readSeoTitleDispatchItems(context, meta.dispatchId);
  if (items.length !== 29) {
    await quarantineDispatch(context, meta, "예약된 전체몰 상품명이 29개가 아니어서 실제 반영을 중단했습니다.", completedAt, "");
  ...
  try {
    ...
    claimed = (await callSeoTitleRpc<boolean>(context, "claim_seo_title_dispatch_direct_apply", {
    ...
    if (!claimed) {
      return { handled: true as const, ok: true as const, duplicateCallback: true as const, directApplyRequestId: "" };
    }
    ...
    return { handled: true as const, ok: true as const, goodsKeys, directApplyRequestId: apply.requestId };
  } catch (error) {
    const message = error instanceof Error ? error.message : "쇼핑몰별 상품명·검색어 반영을 시작하지 못했습니다.";
    await quarantineDispatch(context, meta, message, completedAt, requestId);
    return { handled: true as const, ok: false as const, directApplyRequestId: requestId, claimed };
  }
}

async function quarantineDispatch(
  context: SeoTitleLedgerContext,
  meta: { dispatchId: string; reservationId: string; pipelineVersion: string; canonicalSeed: boolean },
  message: string,
  completedAt: string,
  ...
  try {
    ...
    const finalizationMessage = finalizationError instanceof Error ? finalizationError.message : "상품명 review 격리 처리에 실패했습니다.";
    await Promise.all([
      ...
      patchSeoTitleDispatchItems(context, meta.dispatchId, { status: "submitted", error_message: message }),
    ]);
    return false;
  }
  await Promise.all([
    ...
    patchSeoTitleDispatchItems(context, meta.dispatchId, { status: "failed", error_message: message }),
  ]);
  return true;
}
