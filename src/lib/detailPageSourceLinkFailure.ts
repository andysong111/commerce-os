...
const EXPLICIT_LINK_FAILURE = /SOURCE_LINK_UNAVAILABLE|商品已下架|已下架|商品不存在|页面不存在|页面为空|空白页面|链接失效|无法访问|访问失败|not\s+found|\b404\b|\bgone\b|unavailable|blank\s+page|empty\s+page|page\s+is\s+empty|link.{0,20}unavailable|접근\s*불가|페이지.{0,20}(비어|없음|없습니다|찾을\s*수\s*없)|상품.{0,20}(내려|삭제|존재하지)|링크.{0,20}(만료|실패|접근)/i;
...
export function isDetailPageSourceLinkUnavailable(jobValue: unknown) {
  ...
  // 서버의 pre-AI source gate가 명시적으로 링크 불량을 판정했다면,
  // 과거 수집기가 잘못 저장한 1장의 오류 화면 evidence가 남아 있어도
  // 링크 접근 불가로 표시해야 한다.
