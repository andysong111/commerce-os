...
export async function upsertKeywordEngineElonLabRows(rows: KeywordEngineElonLabStoredRow[]) {
  ...
  // Avoid PostgREST `on_conflict` bulk upserts in this lab. Production returned 503
  // on that route even though reads and Shopling execution succeeded. A row-level
  // PATCH → INSERT fallback is slower but isolates failures and keeps the lab usable.
