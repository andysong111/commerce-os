export const COMMERCE_OS_MARKET_AUTO_BRIDGE_V0330 = String.raw`(() => {
  ...
  window.addEventListener("message", (event) => {
    ...
    try {
      chrome.runtime.sendMessage({ type: BG_HANDOFF, token, orchestrationId }, (response) => {
    ...
    try {
      ...
      if (active && active.token && active.orchestrationId) {
        ...
        if (queue && queue.status === "running" && queueStartedAt >= activeStartedAt) {
          if (Date.now() - Number(active.lastHeartbeatAt || 0) >= HEARTBEAT_MS) {
            const beat = await send({ type: BG_HEARTBEAT, token: active.token, orchestrationId: active.orchestrationId });
            if (beat && beat.ok) await set({ [ACTIVE_KEY]: { ...active, lastHeartbeatAt: Date.now() } });
          }
          return;
        }
        if (queue && queueStartedAt >= activeStartedAt && terminalQueue(queue.status)) {
      ...
      const jobIds = Array.isArray(claim.jobIds)
        ? claim.jobIds.map(text).filter((value) => /^[0-9a-f-]{36}$/i.test(value)).slice(0, 20)
        : [];
