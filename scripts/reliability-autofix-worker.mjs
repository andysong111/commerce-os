...
async function api(body) {
  ...
  if (!response.ok || payload?.ok === false) {
    throw new Error(payload?.message || `autofix API status=${response.status}`);
  }
  return payload;
}
...
function tokens(job) {
  const raw = [job.engine, job.error_code, job.target_test_name, job.title]
    .filter(Boolean)
    .join(" ");
  const result = new Set();
  for (const token of raw.split(/[^A-Za-z0-9_]+/g)) {
    if (token.length >= 4) result.add(token.toLowerCase());
  }
...
function collectContext(job) {
  ...
  for (const absolute of candidates) {
    ...
    for (const token of searchTokens) {
      if (haystack.includes(token)) score += token.includes("_") ? 6 : 2;
    }
  ...
  for (const item of scored) {
    const content = item.content.slice(0, 24_000);
    if (total + content.length > 130_000) continue;
    selected.push({ path: item.path, content });
    total += content.length;
    if (selected.length >= 14) break;
  }
  if (!selected.length) {
    throw new Error("No safe repository context matched the incident");
  }
  return selected;
}
...
function applyProposal(proposal, context) {
  ...
  for (const edit of edits) {
    ...
    if (!oldText) {
      if (!(path.startsWith("tests/") || path.includes(".test.")) || existsSync(absolute)) {
        throw new Error(`New files are test-only: ${path}`);
      }
      mkdirSync(dirname(absolute), { recursive: true });
      writeFileSync(absolute, newText, "utf8");
    } else {
      if (!contextPaths.has(path) || !existsSync(absolute)) {
  ...
  if (!numstat) throw new Error("Autofix proposal produced no diff");
  const changed = [];
  let lineBudget = 0;
  for (const line of numstat.split("\n")) {
    const [addedRaw, deletedRaw, path] = line.split("\t");
    if (!safePath(path)) throw new Error(`Diff touched forbidden path: ${path}`);
    const added = Number(addedRaw);
    const deleted = Number(deletedRaw);
    if (!Number.isFinite(added) || !Number.isFinite(deleted)) {
      throw new Error(`Binary or uncountable diff is forbidden: ${path}`);
    }
    lineBudget += added + deleted;
    changed.push(path);
  }
  if (changed.length > 4 || lineBudget > 260) {
    throw new Error(`Autofix diff exceeds safety budget: files=${changed.length}, lines=${lineBudget}`);
  }
  const sourceChanged = changed.some((path) => path.startsWith("src/") && !path.includes(".test."));
  const testChanged = changed.some((path) => path.startsWith("tests/") || path.includes(".test."));
  if (sourceChanged && !testChanged) {
    throw new Error("Source autofix must include a regression test change");
  }
  return changed;
}

async function claim() {
  const payload = await api({ action: "claim" });
  if (!payload.job) {
    output("has_job", "false");
    return;
  }
  if (payload.job.target_repo !== REPOSITORY) {
    throw new Error("Claimed job repository mismatch");
  }
  writeFileSync(JOB_FILE, JSON.stringify(payload.job, null, 2));
  output("has_job", "true");
  output("job_id", payload.job.job_id);
  output("improvement_id", payload.job.improvement_id);
}
...
async function generate() {
  ...
  const changed = applyProposal(proposal, context);
...
async function finish() {
  const job = readJob();
  const status = process.env.AUTOFIX_STATUS || "failed";
  const changedPaths = process.env.AUTOFIX_CHANGED_PATHS
  ...
  await api({
    ...
    job_id: job.job_id,
    status,
    branch_name: process.env.AUTOFIX_BRANCH || null,
    pr_number: process.env.AUTOFIX_PR ? Number(process.env.AUTOFIX_PR) : null,
    commit_sha: process.env.AUTOFIX_COMMIT || null,
    merge_sha: process.env.AUTOFIX_MERGE || null,
    patch_summary: process.env.AUTOFIX_SUMMARY || null,
    changed_paths: changedPaths,
    error: process.env.AUTOFIX_ERROR || null,
  });
}
