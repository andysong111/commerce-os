...
declare
  ...
  v_error_code text := coalesce(
    ...
    case when upper(coalesce(new.status,''))='FAILED' then 'operation_failed' else null end
  );
...
begin
  perform public.ingest_reliability_event(jsonb_build_object(
    ...
    'error_message',case when v_status in ('failed','blocked','quality_rejected','retrying')
      then nullif(public.redact_reliability_text(new.error_message),'') else null end,
...
begin
  perform public.ingest_reliability_event(jsonb_build_object(
    ...
    'stage',new.stage_key,
    'error_code',v_error_code,
    'error_message',case when v_status in ('failed','quality_rejected','blocked','retrying')
      then nullif(public.redact_reliability_text(coalesce(new.error_message,new.review_note,'')),'') else null end,
    'quality_signals',jsonb_strip_nulls(jsonb_build_object(
      'reviewStatus',case when coalesce(new.review_status,'') ~ '^[A-Za-z0-9_.:-]{1,80}$' then new.review_status else null end,
...
begin
  perform public.ingest_reliability_event(jsonb_build_object(
    ...
    'error_code',v_error_code,
    'error_message',case when v_status in ('failed','blocked','quality_rejected','retrying')
      then nullif(public.redact_reliability_text(coalesce(new.result_payload->>'errorMessage',new.result_payload->>'message','')),'') else null end,
...
-- Optional source tables are guarded so a clean or partially provisioned database
-- can apply the reliability migrations without aborting.
do $$
begin
  if to_regclass('public.commerce_operation_runs') is not null then
    execute 'drop trigger if exists bridge_commerce_operation_run_to_reliability on public.commerce_operation_runs';
    execute 'create trigger bridge_commerce_operation_run_to_reliability after insert or update of status,error_message,result_snapshot on public.commerce_operation_runs for each row execute function public.bridge_commerce_operation_run_to_reliability()';
  end if;
  if to_regclass('public.keyword_engine_elon_lab_stage_results') is not null then
    execute 'drop trigger if exists bridge_keyword_stage_to_reliability on public.keyword_engine_elon_lab_stage_results';
    execute 'create trigger bridge_keyword_stage_to_reliability after insert or update of run_status,review_status,error_message on public.keyword_engine_elon_lab_stage_results for each row execute function public.bridge_keyword_stage_to_reliability()';
  end if;
  if to_regclass('public.product_launch_upload_jobs') is not null then
    execute 'drop trigger if exists bridge_product_launch_upload_to_reliability on public.product_launch_upload_jobs';
    execute 'create trigger bridge_product_launch_upload_to_reliability after insert or update of status,error_message,result on public.product_launch_upload_jobs for each row execute function public.bridge_product_launch_upload_to_reliability()';
  end if;
  if to_regclass('public.seo_title_dispatches') is not null then
    execute 'drop trigger if exists bridge_seo_dispatch_to_reliability on public.seo_title_dispatches';
    execute 'create trigger bridge_seo_dispatch_to_reliability after insert or update of status,result_payload on public.seo_title_dispatches for each row execute function public.bridge_seo_dispatch_to_reliability()';
  end if;
end;
$$;
...
begin
  if to_regclass('public.commerce_operation_runs') is not null then
    for r in execute 'select * from public.commerce_operation_runs' loop
      ...
      v_error_code := coalesce(
        ...
        case when upper(coalesce(r.status,''))='FAILED' then 'operation_failed' else null end
      );
      perform public.ingest_reliability_event(jsonb_build_object(
        ...
        'incident_signature',case when v_status in ('failed','blocked','quality_rejected','retrying') then concat('commerce-operation:',r.operation_type,':',coalesce(v_error_code,v_status)) else null end,
        'stage',coalesce(r.result_snapshot->>'stage',r.input_snapshot->>'stage'),
        'error_code',v_error_code,
        'error_message',case when v_status in ('failed','blocked','quality_rejected','retrying')
          then nullif(public.redact_reliability_text(r.error_message),'') else null end,
  ...
  if to_regclass('public.keyword_engine_elon_lab_stage_results') is not null then
    for r in execute 'select * from public.keyword_engine_elon_lab_stage_results' loop
      ...
      perform public.ingest_reliability_event(jsonb_build_object(
        ...
        'incident_signature',case when v_status in ('failed','quality_rejected','blocked','retrying') then concat('keyword-engine:',r.stage_key,':',coalesce(v_error_code,v_status)) else null end,
        'stage',r.stage_key,'error_code',v_error_code,
        'error_message',case when v_status in ('failed','quality_rejected','blocked','retrying')
          then nullif(public.redact_reliability_text(coalesce(r.error_message,r.review_note,'')),'') else null end,
  ...
  if to_regclass('public.product_launch_upload_jobs') is not null then
    for r in execute 'select * from public.product_launch_upload_jobs' loop
      v_status := public.map_reliability_status(r.status);
      v_error_code := coalesce(
      ...
      perform public.ingest_reliability_event(jsonb_build_object(
        ...
        'incident_signature',case when v_status in ('failed','blocked','quality_rejected','retrying') then concat('product-launch-upload:',coalesce(v_error_code,v_status)) else null end,
  ...
  if to_regclass('public.seo_title_dispatches') is not null then
    for r in execute 'select * from public.seo_title_dispatches' loop
      v_status := public.map_reliability_status(r.status);
      v_error_code := coalesce(
        nullif(r.result_payload->>'errorCode',''),nullif(r.result_payload->>'error_code',''),
        case when v_status='failed' then 'seo_title_dispatch_failed' else null end
      );
      perform public.ingest_reliability_event(jsonb_build_object(
        'event_id',concat('backfill:seo-title-dispatch:',r.dispatch_id::text),
        'source_system','commerce-os','engine','seo-title-cloud-shopling',
        'event_type','seo_title_dispatch.backfill','status',v_status,
        'severity',case when v_status='failed' then 'error' when v_status in ('blocked','quality_rejected','retrying') then 'warning' else 'info' end,
        'risk_level','medium','run_id',r.dispatch_id::text,
        'correlation_id',coalesce(nullif(r.external_request_id,''),r.dispatch_id::text),
        'incident_signature',case when v_status in ('failed','blocked','quality_rejected','retrying') then concat('seo-title-dispatch:',r.dispatch_kind,':',coalesce(v_error_code,v_status)) else null end,
        'stage',r.dispatch_kind,'error_code',v_error_code,
        'error_message',case when v_status in ('failed','blocked','quality_rejected','retrying')
          then nullif(public.redact_reliability_text(coalesce(r.result_payload->>'errorMessage',r.result_payload->>'message','')),'') else null end,
        'metrics',jsonb_build_object('registrationRounds',r.registration_rounds,'requestedTitleCount',r.requested_title_count,'reservedTitleCount',r.reserved_title_count),
        'metadata',jsonb_build_object('dispatchKind',r.dispatch_kind,'launchItemId',r.launch_item_id,'backfilled',true),
        'occurred_at',coalesce(r.completed_at,r.updated_at,r.created_at)
      ));
    end loop;
  end if;
...
revoke all on function public.bridge_commerce_operation_run_to_reliability() from public, anon, authenticated;
revoke all on function public.bridge_keyword_stage_to_reliability() from public, anon, authenticated;
revoke all on function public.bridge_product_launch_upload_to_reliability() from public, anon, authenticated;
revoke all on function public.bridge_seo_dispatch_to_reliability() from public, anon, authenticated;
