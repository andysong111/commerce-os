...
begin
  ...
  if not v_job.retry_scope_known then
    raise exception 'retry failure scope is unknown';
  end if;
  if v_job.retry_round >= v_job.max_retry_rounds then
    raise exception 'maximum retry rounds reached';
  end if;
  if exists (
    select 1
    ...
    raise exception 'active chunk exists';
  end if;

  -- A known/rejected early retry failure terminates the rest of that round.
  -- Superseding stale pending chunks prevents the next approved round from
  -- becoming permanently blocked while preserving uncertain work as active.
  ...
  from (
    select
      goods_key,
      ordinal,
      ((row_number() over (order by ordinal) - 1) / 50)::integer as bucket
    from public.shopling_price_bulk_items
    where job_id = p_job_id and status = 'failed'
  ) failed_only
  group by bucket
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements_text(v_chunk.goods_keys) as key(value)
    where not exists (
      select 1
      from public.shopling_price_bulk_items item
      where item.job_id = p_job_id
        and item.goods_key = key.value
        and item.status = 'failed'
    )
  ) then
    raise exception 'retry chunk contains non-failed item';
  end if;
...
begin
  ...
  if exists (
    select 1
    from unnest(v_failed) as failed_key
    where not exists (
      select 1
      from jsonb_array_elements_text(v_chunk.goods_keys) as chunk_key(value)
      where chunk_key.value = failed_key
    )
  ) then
    raise exception 'failed key outside chunk';
  end if;

  if p_success and cardinality(v_failed) > 0 then
    raise exception 'success cannot include failed keys';
  end if;
  ...
  set status = case when p_success then 'succeeded' else 'failed' end,
      ...
      last_error = case
        ...
        else left(coalesce(p_error, 'retry failed'), 1000)
      end
  where id = v_chunk.id;
  ...
  if not p_success then
    ...
    where original.job_id = v_job.id
      ...
      and not exists (
        ...
        join public.shopling_price_bulk_items item
         ...
         and item.goods_key = original_key.value
    ...
    if v_remaining > 0 then
      ...
      if not exists (
        ...
        raise exception 'canary was not recovered';
      end if;
  ...
  set status = v_status,
      ...
      retry_scope_known = case
        when p_success then true
        else p_failure_scope_known
      end,
      last_error = case
        when p_success then null
        else left(coalesce(p_error, 'retry failed'), 1000)
...
begin
  ...
  if v_chunk.id is null then
    if exists (
      ...
      raise exception 'no reservable normal chunk';
    end if;
...
begin
  ...
  set status = case when p_success then 'succeeded' else 'failed' end,
      ...
      last_error = case
        when p_success then null
        else left(coalesce(p_error, 'normal failed'), 1000)
      end
  where id = v_chunk.id;
...
begin
  ...
  if not exists (
    ...
    raise exception 'canary chunk has not succeeded';
  end if;

  select count(*), coalesce(sum(goods_key_count), 0)
  into v_chunks, v_goods
  from public.shopling_price_bulk_chunks
  ...
  if exists (
    ...
    raise exception 'all normal chunks must be pending';
  end if;
  if exists (
    select 1
    from public.shopling_price_bulk_chunks
