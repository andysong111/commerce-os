...
begin
  if new.application_mode = 'none'
    ...
    new.status := case
      when new.status in ('analysis_pending','approval_required','implementation_needed')
        then new.status
    ...
    new.requires_approval := new.status in (
      'analysis_pending','approval_required','implementation_needed'
    );
...
comment on function public.clear_stale_reliability_application_evidence() is
  'Removes stale application and measurement claims while preserving a stricter analysis, approval, or implementation state computed from current evidence.';
