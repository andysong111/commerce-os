...
begin
  ...
  loop
    ...
    -- The browser can carry the already-issued permanent option barcode while
    -- the operator assigns a B-code later. Never silently replace that number.
    if v_no is not null
       and v_existing_no <> ''
    ...
    -- Identity transition: an option was created before it had a B-code and
    -- therefore used OPTION:<...>. When the B-code is assigned, move the same
    -- registry row to B:<code> so the permanent option barcode number survives.
    if v_no is null
        ...
        if upper(coalesce(v_previous_kind, '')) = 'OPTION'
          ...
          update public.option_barcode_registry
             set identity_key = v_identity,
                 ...
                 updated_at = now()
           where identity_key = v_previous_identity;
          v_no := v_previous_no;
        end if;
      end if;
    end if;
