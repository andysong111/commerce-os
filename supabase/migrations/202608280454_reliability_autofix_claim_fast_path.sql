create or replace function public.claim_reliability_autofix_job(p_repo text, p_run_id text)
returns table(
  job_id uuid,
  improvement_id uuid,
  incident_id uuid,
...
begin
  ...
  -- Fast path: claim an already queued job first. This avoids running the
  -- candidate-enqueue INSERT/count path on every worker wakeup and prevents
  -- an unrelated enqueue lock wait from blocking an approved queued retry.
