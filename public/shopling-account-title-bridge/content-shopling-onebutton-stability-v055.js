(() => {
  ...
  const TITLE_RUN_KEY = "commerceOsShoplingTitleBatchRun";
  ...
  const LEGACY_PURPLE_PANEL_ID = "commerce-os-shopling-product-list-batch";
  const POLL_MS = 4000;
  ...
  async function saveUiRun(run) {
    try {
      ...
      // Supabase market ledger remains authoritative.
    }
  }
  ...
  function suppressLegacyPurplePanel() {
    let style = document.getElementById("commerce-os-shopling-unified-ui-style-v055");
    if (!style) {
      style = document.createElement("style");
      style.id = "commerce-os-shopling-unified-ui-style-v055";
      style.textContent = `#${LEGACY_PURPLE_PANEL_ID}{display:none!important}`;
  ...
  function setPanelBusy(busy, label = "") {
    ...
    button.textContent = label || (busy ? "신규상품 자동처리 진행 중..." : "신규상품 전체 자동처리");
  }
  ...
  async function ensureTitleToMarketHandoff(reason = "poll") {
    ...
    if (!titleRunCoversUiRun(uiRun, lastTitle)) {
      const activeTitleRun = await loadSession(TITLE_RUN_KEY);
      if (text(activeTitleRun?.status) === "running") {
        const total = Array.isArray(activeTitleRun.goodsKeys) ? activeTitleRun.goodsKeys.length : expectedGoodsKeys(uiRun).length;
        setPanelBusy(true, `상품명 점검 ${Number(activeTitleRun.done || 0)}/${total}`);
        setPanelStatus(
          `상품명 점검 진행 ${Number(activeTitleRun.done || 0)}/${total} · ${text(activeTitleRun.currentGoodsKey) || "다음 상품 준비"} · 백그라운드 감시 활성`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (failedKeys.size) {
        for (const task of Array.isArray(fresh.tasks) ? fresh.tasks : []) {
          ...
          await sendRuntimeMessage({
            ...
            message: text(result.message) || "상품명 점검 단계에서 실패하여 마켓 송신을 차단했습니다.",
          });
        }
      }
      ...
      if (response?.ok === true || alreadyRunning) {
        ...
        setPanelStatus(
          alreadyRunning
            ? `마켓 작업 복구 확인 · ${marketTasks.length}건 · 기존 작업 계속 진행`
            : `마켓 자동전송 시작 · ${marketTasks.length}건 · 최대 동시 작업창 2개`,
        );
        return;
      }
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message.type === TITLE_BATCH_PROGRESS_MESSAGE) {
      setTimeout(() => void ensureTitleToMarketHandoff("title-progress-message"), 50);
      return;
    }
    if (message.type === PIPE_MARKET_PROGRESS_MESSAGE) {
      setTimeout(() => void ensureTitleToMarketHandoff("market-progress"), 50);
    }
  });
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (areaName === "session" && (changes[PIPE_UI_RUN_KEY] || changes[TITLE_RUN_KEY])) {
      setTimeout(() => void ensureTitleToMarketHandoff("session-state-change"), 50);
    }
  });

  suppressLegacyPurplePanel();
  const observer = new MutationObserver(() => suppressLegacyPurplePanel());
