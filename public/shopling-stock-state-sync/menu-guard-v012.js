(() => {
  ...
  function stageForLabel(value) {
    ...
    if (/^(?:\[?A?22\]?[:.\-]?)?쇼핑몰상품옵션전송$/i.test(label)) return "A22";
  ...
  async function failActiveJob(message) {
    ...
    if (!active || active.status !== "RUNNING" || !active.job?.jobId || !active.stage) {
      return;
    }
    if (["WAIT_A22_RESULT", "WAIT_A21_RESULT"].includes(String(active.stage))) return;
