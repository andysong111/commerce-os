const VERSION = "0.1.0";
const STATE_KEY = "commerceOsShoplingStockStateSyncV010";
const LAST_RESULT_KEY = "commerceOsShoplingStockStateSyncLastResultV010";
const ALARM_NAME = "commerce-os-shopling-stock-state-watchdog";
...
async function progress(active, message, extra = {}) {
  ...
  active.evidence = [...(active.evidence || []), { at: Date.now(), stage: active.stage, message, ...extra }].slice(-80);
...
async function dispatch(active, tabId, page = null) {
  if (!active || !Number.isInteger(tabId)) return false;
  if (["WAIT_A22_RESULT", "WAIT_A21_RESULT"].includes(active.stage)) return false;
  const expected = expectedRole(active.stage);
  if (active.stage === "A21_POPUP" && page?.role !== "A21_POPUP") return false;
...
async function start(input) {
  ...
  await progress(
    active,
    `${active.job.barcode} ${statusKorean(active.job.desiredStatus)} 동기화 시작 · A6부터 직렬 실행`,
  );
  ...
  await dispatch(active, tab.id, { role: "OTHER" });
  return { ok: true, active, message: active.message };
}
...
async function handleStepResult(message, sender) {
  ...
  if (!result.ok) {
    ...
    if (isTransient(result) && attempts < MAX_TRANSIENT_ATTEMPTS) {
      await progress(
        ...
        { code: result.code, page: message.page },
      );
      await sleep(900);
      if (Number.isInteger(active.shoplingTabId)) {
        await dispatch(active, active.shoplingTabId, message.page);
      }
    ...
    return {
      ...
      result: await finish(
        ...
        { code: result.code || "STOCK_SYNC_STEP_FAILED", page: message.page, detail: result.evidence },
      ),
    };
  }
  ...
  if (result.step === "A6" && result.completed) {
    ...
    if (Number.isInteger(active.shoplingTabId)) {
      await dispatch(active, active.shoplingTabId, { role: message.page?.role || "A6" });
    }
    return { ok: true };
  }
...
async function handlePageReady(message, sender) {
  ...
  if (!active || active.status !== "RUNNING") return { ok: false };
  const tabId = sender?.tab?.id;
  if (!Number.isInteger(tabId) || message.page?.top === false) return { ok: false };
  const page = message.page || {};
  if (active.stage === "A21_POPUP" && page.role === "A21_POPUP") {
    active.shoplingTabId = tabId;
    await saveActive(active);
    await dispatch(active, tabId, page);
    return { ok: true };
  }
  if (["A6", "A22", "A21_LIST"].includes(active.stage)) {
    const expected = expectedRole(active.stage);
    if (page.role === expected || tabId === active.shoplingTabId) {
      active.shoplingTabId = tabId;
      await saveActive(active);
      await dispatch(active, tabId, page);
      return { ok: true };
  ...
  if (Number(evidence.failureCount || 0) > 0 || evidence.explicitFailure) {
    return {
      ...
      result: await finish(
        ...
        { result: evidence, resultTabId: sender?.tab?.id },
      ),
    };
  }
  return {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  ...
  void (async () => {
    ...
    if (tabId !== active.shoplingTabId) return;
    await sleep(500);
    await dispatch(active, tabId, null);
  })();
});
