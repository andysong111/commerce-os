(() => {
  ...
  function role() {
    ...
    if (/\[?A?6\]?\s*옵션대량수정/i.test(text) && /일괄\s*상태변경/i.test(text)) {
      return "A6";
    }
  ...
  async function navigateTo(target) {
    const map = {
      A6: /(?:\[?6\]?|A6)\s*옵션대량수정/i,
      A22: /(?:\[?22\]?|A22)\s*쇼핑몰상품옵션전송/i,
      A21_LIST: /(?:\[?21\]?|A21)\s*쇼핑몰상품수정/i,
    };
    const element = menuTarget(map[target]);
  ...
  async function runA6(job) {
    ...
    if (!selected.ok) {
      return {
        ...
        message: `${job.barcode} 정확 일치 행을 단독 선택하지 못했습니다.`,
      };
    }
  ...
  async function runA22(job) {
    ...
    if (!selected.ok) {
      return {
        ...
        message: `${job.barcode} A22 정확 일치 행을 단독 선택하지 못했습니다.`,
      };
    }
  ...
  async function runA21List(job) {
    ...
    if (!selected.ok) {
      return {
        ...
        message: `${job.modelNo} A21 정확 일치 행을 단독 선택하지 못했습니다.`,
      };
    }
  ...
  async function execute(message) {
    if (!IS_TOP) return { ok: false, ignored: true };
    ...
    let result;
    try {
      if (stage === "A6") result = await runA6(job);
      else if (stage === "A22") result = await runA22(job);
      else if (stage === "A21_LIST") result = await runA21List(job);
      else if (stage === "A21_POPUP") result = await runA21Popup(job);
      else result = { ok: false, code: "STOCK_SYNC_STAGE_INVALID", message: `지원하지 않는 단계 ${stage}` };
    } catch (error) {
      result = {
        ok: false,
