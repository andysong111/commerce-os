(() => {
  ...
  function role() {
    ...
    if (/옵션대량수정/i.test(text) && /일괄\s*상태변경/i.test(text)) return "A6";
  ...
  function findSearchInput(fieldSelect) {
    const candidates = [
      ...(fieldSelect?.closest("tr")?.querySelectorAll('input[type="text"],input:not([type]),textarea') || []),
      ...(fieldSelect?.form?.querySelectorAll('input[type="text"],input:not([type]),textarea') || []),
      ...document.querySelectorAll('input[type="text"],input:not([type]),textarea'),
    ].filter((input, index, all) => all.indexOf(input) === index && !input.disabled && visible(input));
    let best = null;
    let bestScore = -999;
    for (const input of candidates) {
      const context = localText(input);
      let score = 0;
      if (/검색항목|다중검색|자사상품코드|옵션자체관리코드|모델번호|상품등록번호|상품검색용코드|상품번호|상품키/i.test(context)) score += 15;
  ...
  async function searchExact(fieldLabel, token) {
    const fieldSelect = selectWithOption(fieldLabel)[0] || null;
    ...
    if (!input || !setInput(input, token)) {
      return { ok: false, code: "SEARCH_INPUT_NOT_FOUND", message: `${fieldLabel} 검색 입력칸을 찾지 못했습니다.` };
    }
  ...
  async function searchGoodsKey(goodsKey) {
    const labels = ["상품등록번호", "상품검색용코드", "상품번호", "상품키", "Goods Key", "goods key"];
  ...
  async function runA6(job) {
    ...
    if (!button) return { ok: false, code: "A6_BULK_STATUS_BUTTON_NOT_FOUND", message: "A6 일괄 상태변경 버튼을 찾지 못했습니다." };
  ...
  function retryable(result) {
    return Boolean(
      result?.navigating ||
        result?.waiting ||
        ["SEARCH_FIELD_NOT_FOUND", "SEARCH_INPUT_NOT_FOUND", "SEARCH_BUTTON_NOT_FOUND", "SHOPLING_MENU_NOT_FOUND", "A21_POPUP_WAIT"].includes(String(result?.code || "")),
    );
  }
