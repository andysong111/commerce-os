(() => {
  ...
  async function reconcileStaleRunningState(response) {
    ...
    const sameJob = Boolean(active?.job?.jobId && lastResult?.jobId && String(active.job.jobId) === String(lastResult.jobId));
    const staleTerminalRunning = Boolean(
      active?.status === "RUNNING" && sameJob && isTerminalOutcome(lastResult?.outcome) && activeStartedAt > 0 && lastFinishedAt >= activeStartedAt,
    );
  ...
  const announce = async () => {
    ...
    if (reconciled.lastResult && Date.now() - Number(reconciled.lastResult.finishedAt || 0) < 3_600_000) {
  ...
  window.addEventListener("message", (event) => {
    ...
    void chrome.runtime.sendMessage({ type: "STOCK_SYNC_START", job: data.job }).then((response) => {
      ...
      post({
        ...
        evidence: { code: response?.code || "STOCK_SYNC_START_FAILED", missing: response?.missing || [] },
        finishedAt: Date.now(),
      });
    }).catch((error) => {
      post({
