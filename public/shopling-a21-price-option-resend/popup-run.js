...
async function loadPlan() {
  try {
    ...
    planCard.innerHTML = `<div class="grid"><div class="metric">GOODSKEY<b>${plan.goodsKeyCount}</b></div><div class="metric">검증 쇼핑몰가격<b>${plan.readback.mallMatchCount}/${plan.readback.mallCheckCount}</b></div></div><p class="ok" style="margin-top:8px;font-weight:700">Shopling 가격 재조회 VERIFIED · 불일치 ${plan.readback.mallMismatchCount} · 누락 ${plan.readback.mallMissingCount}</p><p style="margin-top:5px">v0.4.0은 A21 최종전송일을 완료판정에 사용하지 않습니다. 실제 Shopling 결과문서 자체에 감지기를 주입하고, about:blank 및 상속 origin 프레임까지 포함해 결과문서 내부에서 ‘처리중입니다/잠시만 기다려주시기 바랍니다’를 직접 관찰합니다. 감지기가 붙은 결과문서는 자동으로 아래까지 스크롤하여 완료 footer도 보조 신호로 확보합니다. 실제 로딩 종료가 확인된 뒤에만 기존 v0.2.7의 completeJob → 다음 큐 구조로 옵션전송을 시작합니다. 마켓별 성공/실패는 진행 조건으로 사용하지 않습니다.</p>`;
    setRunButtons(true);
  } catch (error) {
