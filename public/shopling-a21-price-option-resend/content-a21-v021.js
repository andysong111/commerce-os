(() => {
  ...
  const MAIN_SUBMIT_MESSAGE = "A21_MAIN_SUBMIT_V021";
  ...
  function overlay() {
    let node = document.getElementById("commerce-os-a21-v021-status");
    if (node) return node;
    node = document.createElement("div");
    node.id = "commerce-os-a21-v021-status";
    node.style.cssText = "position:fixed;right:12px;top:12px;z-index:2147483647;max-width:380px;padding:10px 12px;border-radius:10px;background:#0f172a;color:#fff;font:12px/1.4 Arial,sans-serif;box-shadow:0 8px 24px rgba(15,23,42,.25)";
    node.textContent = "Commerce OS v0.2.1 · 송신 작업 연결 대기";
  ...
  function status(text, tone = "dark") {
    const node = overlay();
    node.textContent = `Commerce OS v0.2.1 · ${text}`;
  ...
  async function configurePrice() {
    ...
    if (!await chooseMode("goods_normal")) return { ok: false, code: "V021_PRICE_MODE", message: "modify_tp=goods_normal 선택 실패" };
    const source = hiddenValues("tsmt_sale_price_tp");
    if (!source.length || source.some((value) => value !== "J")) {
      return { ok: false, code: "V021_PRICE_SOURCE", message: `tsmt_sale_price_tp가 J가 아닙니다: ${source.join(",") || "없음"}` };
    }
    for (const name of GENERAL_NAMES) {
      const expected = name === "trsmt_env_mody_price" ? "Y" : "";
      if (!await selectRadio(name, expected)) return { ok: false, code: "V021_GENERAL_SELECT", message: `${name}=${expected || "(blank)"} 선택 실패` };
    }
    const wrong = GENERAL_NAMES.filter((name) => !verifyRadio(name, name === "trsmt_env_mody_price" ? "Y" : ""));
    if (!verifyRadio("modify_tp", "goods_normal") || wrong.length) return { ok: false, code: "V021_PRICE_VERIFY", message: `판매가 단독 검증 실패: ${wrong.join(",") || "modify_tp"}` };
    return { ok: true };
  }

  async function configureOption() {
    status("옵션 전송 form 설정 중");
    if (!await chooseMode("goods_stock")) return { ok: false, code: "V021_OPTION_MODE", message: "modify_tp=goods_stock 선택 실패" };
    await sleep(180);
    if (!await selectRadio("trsmt_env_mody_opt", "1")) return { ok: false, code: "V021_OPTION_SELECT", message: "trsmt_env_mody_opt=1 선택 실패" };
    if (!verifyRadio("modify_tp", "goods_stock") || !verifyRadio("trsmt_env_mody_opt", "1")) return { ok: false, code: "V021_OPTION_VERIFY", message: "옵션송신 단독 검증 실패" };
    return { ok: true };
  }
  ...
  async function configureAndSubmit(assignment) {
    ...
    busy = true;
    try {
      if (!isExactPopupUrl()) return sendFailure(assignment.jobId, "V021_POPUP_URL", `송신 URL 불일치: ${location.href}`);
      if (!payloadExists()) return sendFailure(assignment.jobId, "V021_PAYLOAD", "prod_join_chk[] 대상값이 없어 송신 차단");
      ...
      if (!valid) return sendFailure(assignment.jobId, "V021_PRE_SUBMIT_VERIFY", "송신 직전 form 상태 검증 실패");

      status(`${assignment.mode === "PRICE" ? "판매가" : "옵션"} 설정 완료 · 1.2초 후 MAIN world 송신`, "ok");
      ...
      if (!stillValid) return sendFailure(assignment.jobId, "V021_CONFIG_CHANGED", "대기 중 form 상태가 바뀌어 송신 차단");

      status("Shopling 원본 송신함수 MAIN world 호출 예약", "warn");
      await sendStage(assignment.jobId, "SUBMIT_CLICKED", `${assignment.mode === "PRICE" ? "판매가" : "옵션"} · MAIN world 상품수정 송신 호출`);
      const response = await chrome.runtime.sendMessage({ type: MAIN_SUBMIT_MESSAGE, jobId: assignment.jobId, mode: assignment.mode }).catch((error) => ({ ok: false, error: error?.message || String(error) }));
      if (!response?.ok) return sendFailure(assignment.jobId, "V021_MAIN_SUBMIT_FAILED", `MAIN world 송신 실패: ${response?.error || "응답 없음"}`);
      status("Shopling 원본 goods_mallMdfy_submit_sp() 실행", "ok");
    ...
    void (async () => {
      try {
        if (message?.type === "A21_IDENTIFY") {
          sendResponse({ ok: true, role: isExactPopupUrl() ? "A21_POPUP" : "OTHER", version: VERSION });
          return;
        }
        ...
        if (message?.jobId) await sendFailure(message.jobId, "V021_CONTENT_EXCEPTION", error instanceof Error ? error.message : String(error));
