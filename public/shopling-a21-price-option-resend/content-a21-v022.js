(() => {
  ...
  function overlay() {
    let node = document.getElementById("commerce-os-a21-v022-status");
    if (node) return node;
    node = document.createElement("div");
    node.id = "commerce-os-a21-v022-status";
    node.style.cssText = "position:fixed;right:12px;top:12px;z-index:2147483647;max-width:390px;padding:10px 12px;border-radius:10px;background:#0f172a;color:#fff;font:12px/1.4 Arial,sans-serif;box-shadow:0 8px 24px rgba(15,23,42,.25)";
    node.textContent = "Commerce OS v0.2.2 · 송신 작업 연결 대기";
  ...
  function status(text, tone = "dark") {
    const node = overlay();
    node.textContent = `Commerce OS v0.2.2 · ${text}`;
  ...
  async function configurePrice() {
    ...
    if (!await chooseMode("goods_normal")) return { ok: false, code: "V022_PRICE_MODE", message: "modify_tp=goods_normal 선택 실패" };
    const source = hiddenValues("tsmt_sale_price_tp");
    if (!source.length || source.some((value) => value !== "J")) return { ok: false, code: "V022_PRICE_SOURCE", message: `tsmt_sale_price_tp가 J가 아닙니다: ${source.join(",") || "없음"}` };
    for (const name of GENERAL_NAMES) {
      const expected = name === "trsmt_env_mody_price" ? "Y" : "";
      if (!await selectRadio(name, expected)) return { ok: false, code: "V022_GENERAL_SELECT", message: `${name}=${expected || "(blank)"} 선택 실패` };
    }
    if (!verifyPrice()) return { ok: false, code: "V022_PRICE_VERIFY", message: "판매가만 수정 + 배송정보 포함 나머지 수정안함 검증 실패" };
    return { ok: true };
  }

  async function configureOption() {
    status("옵션 전송 form 설정 중");
    if (!await chooseMode("goods_stock")) return { ok: false, code: "V022_OPTION_MODE", message: "modify_tp=goods_stock 선택 실패" };
    await sleep(180);
    if (!await selectRadio("trsmt_env_mody_opt", "1")) return { ok: false, code: "V022_OPTION_SELECT", message: "trsmt_env_mody_opt=1 선택 실패" };
    if (!verifyOption()) return { ok: false, code: "V022_OPTION_VERIFY", message: "옵션송신 단독 검증 실패" };
    return { ok: true };
  }
  ...
  function verifyPrice() {
    ...
    return verifyRadio("trsmt_env_mody_dlvyinfo", "");
  }
  ...
  function mainSubmit(mode) {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        if (settled) return;
        cleanup();
        resolve({ ok: false, error: "v022_main_bridge_timeout" });
      }, 5000);
    });
  }
  ...
  async function configureAndSubmit(assignment) {
    ...
    busy = true;
    try {
      if (!isExactPopupUrl()) return sendFailure(assignment.jobId, "V022_POPUP_URL", `송신 URL 불일치: ${location.href}`);
      if (!payloadExists()) return sendFailure(assignment.jobId, "V022_PAYLOAD", "prod_join_chk[] 대상값이 없어 송신 차단");
      ...
      if (!valid) return sendFailure(assignment.jobId, "V022_PRE_SUBMIT_VERIFY", "송신 직전 form 상태 검증 실패");

      status(`${assignment.mode === "PRICE" ? "판매가" : "옵션"} 설정 완료 · 1.2초 후 MAIN world 원본송신`, "ok");
      ...
      if (!stillValid) return sendFailure(assignment.jobId, "V022_CONFIG_CHANGED", "대기 중 form 상태가 바뀌어 송신 차단");
      ...
      if (!response?.ok) return sendFailure(assignment.jobId, "V022_MAIN_SUBMIT_FAILED", `MAIN world 송신 실패: ${response?.error || "응답 없음"}`);
      if (assignment.mode === "PRICE" && response.deliveryInfoUnchanged !== true) return sendFailure(assignment.jobId, "V022_DELIVERY_GUARD", "배송정보 수정안함 검증값을 확인하지 못해 성공 처리하지 않습니다.");
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    void (async () => {
      try {
        ...
        if (message?.jobId) await sendFailure(message.jobId, "V022_CONTENT_EXCEPTION", error instanceof Error ? error.message : String(error));
