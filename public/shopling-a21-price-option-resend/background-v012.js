...
function transientFrameError(message) {
  return /about:blank|Cannot access contents of url|Frame with ID .* was removed|No frame with id|The frame was removed|Cannot access a chrome:\/\/ URL|Cannot access a data: URL/i.test(message);
}
...
async function executeAllFrames(tabId, func, args = []) {
  try {
    ...
    if (transientFrameError(message)) return [];
    if (/No tab with id|The extensions gallery cannot/i.test(message)) throw error;
    return [];
  }
}
...
async function clickA21Menu(tabId) {
  const results = await executeAllFrames(tabId, () => {
    ...
    const candidate = nodes.find((node) => {
      const value = node instanceof HTMLInputElement ? `${node.value || ""} ${node.title || ""}` : `${node.textContent || ""} ${node.getAttribute("title") || ""}`;
...
async function injectContentIfMissing(tabId, frameId) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error || "content injection failed");
    if (transientFrameError(message)) throw new Error(`TRANSIENT_CONTENT_INJECTION:${message}`);
    throw error;
  }
  await sleep(120);
}

async function sendFrame(tabId, frameId, message, attempts = 30, delay = 250) {
  let lastError = null;
...
async function waitForA21ListFrame(tabId) {
  ...
  while (now() < deadline) {
    ...
    if (!tab) throw new Error("A21 작업 탭이 닫혔습니다.");
    const url = String(tab.url || "");
    if (url && !url.startsWith(SHOPLING_ORIGIN) && url !== "about:blank") {
    ...
    if (!clicked && url.startsWith(SHOPLING_ORIGIN)) clicked = await clickA21Menu(tabId);
    await sleep(clicked ? 500 : TRANSIENT_RETRY_MS);
  }
...
async function waitForPopupFrame(tabId, allowResult = false) {
  ...
  while (now() < deadline) {
    const tab = await chrome.tabs.get(tabId).catch(() => null);
    if (!tab) throw new Error("Shopling 상품수정 송신 팝업 탭이 닫혔습니다.");
    const url = String(tab.url || "");
    if (!url || url === "about:blank" || url.startsWith("data:")) {
      await sleep(TRANSIENT_RETRY_MS);
      continue;
    }
    if (!url.startsWith(SHOPLING_ORIGIN)) {
      await sleep(TRANSIENT_RETRY_MS);
      continue;
    }
    const frames = await identifyFrames(tabId);
...
async function assignWorkerTab(tabId) {
  ...
  if (!job || job.assignmentBusy) return;
  ...
  try {
    ...
    if (["A21_BOOTSTRAP", "OPENING", "SEARCH_CONFIG"].includes(current.stage)) current.stage = "SEARCH_CONFIG";
...
async function discoverPopupForJob(jobId) {
  const deadline = now() + FRAME_WAIT_MS;
  while (now() < deadline) {
    const state = await loadState();
    const job = state?.jobs?.find((item) => item.id === jobId);
    if (!state || !job || job.status !== "RUNNING") return;
    if (Number.isInteger(job.popupTabId)) {
      await assignPopupTab(job.popupTabId, job.workerTabId);
      return;
    }
    const tabs = await chrome.tabs.query({});
    const claimed = new Set(state.jobs.map((item) => item.popupTabId).filter(Number.isInteger));
    const openerMatches = tabs.filter((tab) => tab.id && !claimed.has(tab.id) && tab.openerTabId === job.workerTabId);
    if (openerMatches.length === 1) {
      await assignPopupTab(openerMatches[0].id, job.workerTabId);
      return;
    }
    const popupOpeningJobs = state.jobs.filter((item) => item.status === "RUNNING" && item.stage === "POPUP_OPENING" && !Number.isInteger(item.popupTabId));
    if (popupOpeningJobs.length === 1) {
      const roleMatches = [];
      for (const tab of tabs) {
        if (!tab.id || claimed.has(tab.id) || tab.id === job.workerTabId) continue;
        if (!String(tab.url || "").startsWith(SHOPLING_ORIGIN)) continue;
        const frameId = frameByRole(await identifyFrames(tab.id), "A21_POPUP");
        if (Number.isInteger(frameId)) roleMatches.push(tab);
      }
      if (roleMatches.length === 1) {
        await assignPopupTab(roleMatches[0].id, job.workerTabId);
        return;
      }
    }
    await sleep(TRANSIENT_RETRY_MS);
  }
  await failJob(jobId, "POPUP_DISCOVERY_TIMEOUT", "상품수정 송신 팝업은 열렸지만 작업창과 안전하게 연결하지 못했습니다.");
}
...
async function assignPopupTab(tabId, openerTabId) {
  ...
  if (!job || job.popupAssignmentBusy) return;
...
chrome.tabs.onCreated.addListener((tab) => {
  ...
  void (async () => {
    ...
    if (!state || state.state !== "RUNNING") return;
    if (tab.openerTabId && state.jobs.some((job) => job.workerTabId === tab.openerTabId && job.status === "RUNNING")) {
      await assignPopupTab(tab.id, tab.openerTabId);
    }
  })();
});
  ...
  void (async () => {
    try {
      if (message?.type === "A21_GET_PLAN") {
        sendResponse({ ok: true, plan: await fetchPlan() });
        return;
      }
      ...
      if (message?.type === "A21_STAGE") {
        ...
        if (state && job && job.status === "RUNNING") {
          ...
          if (job.stage === "POPUP_OPENING") {
            setTimeout(() => void discoverPopupForJob(job.id), 100);
          }
