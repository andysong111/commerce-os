...
(() => {
  ...
  async function loadStateV042() {
    const stored = await chrome.storage.local.get(STATE_KEY);
    return stored[STATE_KEY] || null;
  }

  async function saveStateV042(state) {
  ...
  async function attachV042(tabId) {
    try {
  ...
  async function detachV042(tabId) {
  ...
  async function probeDefinitive(tabId) {
    ...
    const attached = await attachV042(tabId);
  ...
  completeJob = async function completeJobV042(jobId, priorMessage) {
    const initialState = await loadStateV042();
    ...
    // v0.4.1 could call completeJob as soon as partial result tables appeared.
    // v0.4.2 deliberately blocks that transition until the same result document
    // has reached the final Shopling footer AND document.readyState === complete.
    ...
    while (Date.now() - startedAt < WAIT_LIMIT_MS) {
      ...
      const definitive = probe.footer === true && probe.readyState === "complete";
      if (!definitive) {
        stableSince = 0;
        const footerLabel = probe.footer ? "완료문구 있음" : "완료문구 대기";
        job.message = `${job.mode === "PRICE" ? "판매가" : "옵션"} 조기완료 차단 · ${footerLabel} · document ${probe.readyState || "unknown"} · 최종완료 전 옵션전송 금지 v${VERSION}`;
        await saveStateV042(state);
        await sleep(POLL_MS);
        continue;
      }
      ...
      job.message = `${job.mode === "PRICE" ? "판매가" : "옵션"} 최종완료 확정 중 · 완료 footer + document complete · ${Math.min(stableMs, STABLE_MS)}/${STABLE_MS}ms v${VERSION}`;
      await saveStateV042(state);
      ...
      if (stableMs >= STABLE_MS) {
        ...
        return baseCompleteJobV041(
          jobId,
          `${job.mode === "PRICE" ? "판매가" : "옵션"} 수정전송 완료 · Shopling 최종 footer + document.readyState complete 확인 후 다음 큐 진행 · 조기 옵션전송 차단 v${VERSION}`,
        );
      }
      await sleep(POLL_MS);
    }
    ...
    if (typeof failJob === "function") {
      return failJob(jobId, "V042_DEFINITIVE_COMPLETION_TIMEOUT", "Shopling 결과창의 최종 완료 footer와 document complete 상태를 30분 동안 확인하지 못했습니다.");
    }
  };

  const baseStartRunV041 = startRun;
  startRun = async function startRunV042(sourceTabId, testMode = false) {
    const result = await baseStartRunV041(sourceTabId, testMode);
    const state = await loadStateV042();
    if (!state) return result;
    state.version = VERSION;
    state.resultPolicy = "CDP_DEFINITIVE_FOOTER_AND_DOCUMENT_COMPLETE";
    await saveStateV042(state);
