...
function refreshButton(button, status) {
  ...
  button.disabled = !ready || !currentItemId || polling;
  if (!ready) status.textContent = "누락값을 보완하면 실제 등록할 수 있습니다.";
  else if (!polling) status.textContent = "미리보기를 확인한 뒤 실행하세요.";
}

async function startUpload(button, status) {
  if (!currentItemId || polling) return;
...
async function pollJob(status) {
  ...
  for (let poll = 0; poll < maxPolls; poll += 1) {
    ...
    if (job.status === "partial_failure" || job.status === "failed") {
      ...
      status.textContent = message || "일부 채널 등록에 실패했습니다. 결과를 확인하세요.";
      polling = false;
      return;
    }
  }
