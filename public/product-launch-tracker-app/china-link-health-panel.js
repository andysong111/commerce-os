...
function installStyles() {
  ...
  style.textContent = `
    ...
    .china-link-health-title { color:#7f1d1d; font-size:14px; font-weight:900; }
    .china-link-health-copy { margin-top:4px; color:#64748b; font-size:11px; line-height:1.5; }
    .china-link-health-stats { display:grid; grid-template-columns:repeat(5,minmax(100px,1fr)); gap:8px; margin-top:12px; }
    .china-link-health-stat { padding:10px 11px; border:1px solid #fed7aa; border-radius:10px; background:#fff; }
    .china-link-health-stat span { display:block; color:#64748b; font-size:10px; font-weight:800; }
    .china-link-health-stat strong { display:block; margin-top:3px; color:#0f172a; font-size:18px; font-weight:950; }
    .china-link-health-actions { display:flex; gap:7px; flex-wrap:wrap; margin-top:12px; }
    .china-link-health-actions .button { min-height:34px; padding:7px 11px; white-space:nowrap; }
    .china-link-health-message { margin-top:10px; padding:9px 11px; border-radius:9px; background:#fff; color:#7c2d12; font-size:11px; font-weight:750; line-height:1.5; }
    .china-link-health-progress { margin-top:8px; height:8px; overflow:hidden; border-radius:999px; background:#ffedd5; }
    .china-link-health-progress > i { display:block; height:100%; background:#f97316; transition:width .2s ease; }
    .china-link-health-dialog { width:min(1120px,94vw); max-height:86vh; padding:0; border:0; border-radius:16px; box-shadow:0 24px 70px rgb(15 23 42 / 35%); }
    .china-link-health-dialog::backdrop { background:rgb(15 23 42 / 48%); }
    .china-link-health-dialog-head { position:sticky; top:0; z-index:2; display:flex; justify-content:space-between; gap:12px; padding:16px 18px; background:#fff; border-bottom:1px solid #e2e8f0; }
    .china-link-health-dialog-body { padding:15px 18px 20px; background:#f8fafc; }
    .china-link-health-table { width:100%; border-collapse:collapse; background:#fff; font-size:11px; }
    .china-link-health-table th,.china-link-health-table td { padding:9px 8px; border-bottom:1px solid #e2e8f0; text-align:left; vertical-align:top; }
    .china-link-health-table th { position:sticky; top:0; background:#f1f5f9; color:#475569; }
    .china-link-health-url { max-width:320px; overflow-wrap:anywhere; color:#475569; }
    .china-link-health-error { color:#be123c; font-weight:800; }
...
function installPanel() {
  if (document.querySelector("#china-link-health-panel")) {
    panel = document.querySelector("#china-link-health-panel");
    return true;
  }
  ...
  panel.innerHTML = `
    <div class="china-link-health-head">
      <div>
        <div class="china-link-health-title">고정링크 1번 상태 · 저부하 브라우저 검사</div>
        <div class="china-link-health-copy">페이지 로딩 때 1688을 자동 호출하지 않습니다. 버튼을 누른 경우에만 Collector가 별도 창에서 링크를 한 개씩 검사하고, 결과만 저장합니다.</div>
      </div>
      <span id="china-link-health-collector" class="status-badge">Collector 확인 중</span>
    </div>
    ...
    <div id="china-link-health-message" class="china-link-health-message">고정링크 상태 요약을 불러오는 중입니다.</div>
    <div class="china-link-health-progress" hidden><i style="width:0%"></i></div>
  `;
  if (notice?.parentElement) {
    notice.parentElement.insertBefore(panel, workspace);
  } else {
...
function installDialog() {
  ...
  dialog.innerHTML = `
    <div class="china-link-health-dialog-head">
      <div><strong>고정링크 1번 오류 목록</strong><div class="china-link-health-copy">선택 상품의 오류 링크를 삭제하고 2번 이후 링크를 한 칸씩 올립니다.</div></div>
      <button type="button" class="icon-button" data-health-dialog="close" aria-label="닫기">×</button>
    </div>
    <div class="china-link-health-dialog-body">
      <div class="china-link-health-actions">
        <button type="button" class="button button-secondary" data-health-dialog="select-all">전체 선택</button>
        <button type="button" class="button button-danger" data-health-dialog="shift-selected">선택 오류 링크 삭제·승격</button>
        <button type="button" class="button button-danger" data-health-dialog="shift-all">오류 전체 삭제·승격</button>
      </div>
      <div id="china-link-health-dialog-message" class="china-link-health-message">오류 목록을 불러오는 중입니다.</div>
      <div style="overflow:auto;max-height:60vh;margin-top:10px"><table class="china-link-health-table"><thead><tr><th></th><th>모델</th><th>오류</th><th>고정링크 1</th><th>승격될 2번</th><th>검사시각</th></tr></thead><tbody id="china-link-health-error-body"></tbody></table></div>
    </div>
  `;
...
function renderSummary(next) {
  ...
  setMessage(
    `오류 ${number(summary.linkErrors)}건 · 후순위 링크가 있어 즉시 승격 가능한 오류 ${number(summary.linkErrorsWithFallback)}건 · 마지막 검사 ${dateText(summary.lastCheckedAt)}`,
  );
}
...
function setAuditUi(running, completed = 0, total = 0) {
  ...
  const buttons = panel?.querySelectorAll("button[data-health-action]") || [];
  ...
  return new Promise((resolve, reject) => {
    ...
    const onMessage = (event) => {
      if (
        ...
        event.source !== worker ||
        event.data?.source !== CALLBACK_SOURCE ||
        event.data?.type !== "china-link-audit-result"
      ) {
        return;
      }
      try {
        const parsed = decodeBase64Utf8(event.data.encoded);
        finish(parsed);
      } catch (error) {
        reject(error);
      }
    ...
    const timeout = window.setTimeout(() => {
      finish({
        ...
        errorMessage: "1688 검사 결과가 제한시간 안에 돌아오지 않았습니다. 링크 오류로 확정하지 않았습니다.",
        finalUrl: "",
...
async function runAudit(scope) {
  ...
  const worker = window.open(
    "about:blank",
    WORKER_NAME,
    "popup=yes,width=540,height=760,left=60,top=60",
  );
  if (!worker) {
  ...
  try {
    for (const row of rows) {
      if (auditCancelled) break;
      if (worker.closed) {
        auditCancelled = true;
        setMessage("검사창이 닫혀 저속 검사를 중단했습니다.", true);
        break;
      }
      const context = {
        mode: "link_audit",
      ...
      try {
        ...
        worker.location.replace(row.primary_url);
      } catch {
        auditCancelled = true;
        setMessage("검사창을 제어하지 못했습니다. 창을 닫고 다시 시작하세요.", true);
        break;
      }

      setMessage(
        `${completed + 1}/${rows.length} · ${row.model_number || row.product_name || row.item_id} 고정링크 1번 검사 중 · 오류 ${permanentErrors}건`,
      );
      ...
      await delay(BETWEEN_LINK_DELAY_MS);
    }
    await flushAuditResults(true);
    if (!auditCancelled) {
      setMessage(
        `저속 검사 완료 · ${completed}건 · 고정링크 오류 ${permanentErrors}건 · 일시 오류 ${temporaryErrors}건`,
      );
    }
  } catch (error) {
...
async function loadErrors(openDialog = true) {
  ...
  try {
    ...
    if (message) {
      message.textContent = error instanceof Error ? error.message : "오류 목록을 읽지 못했습니다.";
    }
  }
}
...
function renderErrors() {
  ...
  message.textContent = errorRows.length
    ? `고정링크 1번 오류 ${errorRows.length}건 · 후순위 링크 있음 ${errorRows.filter((row) => row.has_fallback).length}건`
    : "현재 확정된 고정링크 1번 오류가 없습니다.";
  body.innerHTML = errorRows
    .map(
      (row) => `
        <tr>
          <td><input type="checkbox" data-health-error-id="${escapeHtml(row.item_id)}" /></td>
          <td><strong>${escapeHtml(row.model_number || "—")}</strong><br>${escapeHtml(row.product_name || "")}</td>
          <td class="china-link-health-error">${escapeHtml(row.error_message || row.error_code || "링크 오류")}</td>
          <td class="china-link-health-url"><a href="${escapeHtml(row.primary_url)}" target="_blank" rel="noreferrer">${escapeHtml(row.primary_url)}</a></td>
          <td class="china-link-health-url">${row.fallback_url ? `<a href="${escapeHtml(row.fallback_url)}" target="_blank" rel="noreferrer">${escapeHtml(row.fallback_url)}</a>` : "후순위 링크 없음"}</td>
          <td>${escapeHtml(dateText(row.checked_at))}</td>
        </tr>`,
    )
    .join("");
}
...
function selectedErrorRows(all = false) {
  ...
  const selected = new Set(
    [...(dialog?.querySelectorAll("[data-health-error-id]:checked") || [])]
      .map((input) => input.dataset.healthErrorId || "")
      .filter(Boolean),
  );
...
async function shiftErrorRows(rows) {
  ...
  const confirmed = window.confirm(
    `${rows.length}건의 고정링크 1번을 삭제합니다.\n후순위 링크 승격 ${withFallback}건 · 링크가 비게 되는 상품 ${withoutFallback}건\n계속하시겠습니까?`,
  );
  if (!confirmed) return;
  ...
  try {
    await requestJson(SHIFT_API, {
      ...
      body: JSON.stringify({
        ...
        expectedPrimaryUrls: Object.fromEntries(
          rows.map((row) => [row.item_id, row.primary_url]),
        ),
        updatedBy: "승준",
    ...
    if (message) {
      message.textContent = `정리 완료 · ${rows.length}건 · 후순위 링크 승격 ${withFallback}건 · 링크 없음 ${withoutFallback}건`;
    }
    await Promise.all([loadErrors(false), loadSummary()]);
  } catch (error) {
    if (message) {
      message.textContent = error instanceof Error ? error.message : "고정링크 순서를 정리하지 못했습니다.";
    }
  }
}
...
function handlePanelClick(event) {
  ...
  if (action === "cancel") {
    ...
    setMessage("검사를 중단하는 중입니다. 이미 저장된 결과는 유지됩니다.");
  }
}

function handleDialogClick(event) {
  const button = event.target instanceof Element
    ? event.target.closest("button[data-health-dialog]")
    : null;
  if (!(button instanceof HTMLButtonElement)) return;
  const action = button.dataset.healthDialog;
  if (action === "close") dialog?.close();
  if (action === "select-all") {
    dialog?.querySelectorAll("[data-health-error-id]").forEach((input) => {
