...
async function enrichReplaceMutation(mutation) {
  ...
  const currentOptions = Array.isArray(current?.orderOptions) ? current.orderOptions : [];
  const expectedOptions = Array.isArray(expected.orderOptions) ? expected.orderOptions : [];

  const orderOptions = expectedOptions.map((option, index) => {
...
async function verifyPersistedItem(itemId, expected) {
  ...
  for (const delay of VERIFY_DELAYS) {
    ...
    try {
      ...
      const result = compareEditableItem(expected, actual);
...
function compareEditableItem(expected, actual) {
  ...
  const expectedOptions = Array.isArray(expected?.orderOptions) ? expected.orderOptions : [];
  const actualOptions = Array.isArray(actual?.orderOptions) ? actual.orderOptions : [];
